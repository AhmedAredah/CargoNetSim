...
MainWindow::MainWindow() : CustomMainWindow() {
    // Initialize region management
    Backend::RegionDataController::getInstance().addRegion("Default Region");
    Backend::RegionDataController::getInstance().setRegionVariable("Default Region", "color", QColor(Qt::green));
    Backend::RegionDataController::getInstance().setCurrentRegion("DefaultRegion");
    ...
    QColor regionColor =
        Backend::RegionDataController::getInstance()
            .getRegionVariableAs<QColor>("Default Region", "color");
    ...
    // Set window title
    setWindowTitle("CargoNetSim: Multimodal Freight Operations Optimizer");
    resize(1000, 700);
...
MainWindow::~MainWindow() {
    ...
    // Scene items will be cleaned up by Qt's parent-child mechanism
}
...
void MainWindow::initializeUI() {
    ...
    QVBoxLayout* mainViewLayout = new QVBoxLayout(mainViewTab);
    ...
    QVBoxLayout* globalMapLayout = new QVBoxLayout(globalMapTab);
    ...
    for (const QString& connType : connectionTypes_) {
        QAction* action = connectionMenu_->addAction(connType);
        action->setCheckable(true);
        connect(action, &QAction::triggered, 
                [this, connType](bool checked) { setConnectionType(connType); });
    }
...
void MainWindow::setupDocks() {
    ...
    // Settings dock
    settingsDock_ = new QDockWidget("Simulation Settings", this);
    ...
    shortestPathTableDock_ = new QDockWidget("Shortest Paths Table", this);
    ...
    shortestPathTableDock_->hide();  // Start with shortest paths table hidden
    
    // Connect the settingsChanged signal
    connect(settingsWidget_, &SettingsWidget::settingsChanged, 
            [this](const QMap<QString, QVariant>& settings) {
                showStatusBarMessage("Simulation settings updated.", 2000);
            });
    ...
    addDockWidget(Qt::LeftDockWidgetArea, networkManagerDock_);
    networkManagerDock_->resize(networkManagerDock_->width(), 200);
}

void MainWindow::setupTerminalLibrary() {
    libraryDock_ = new QDockWidget("Terminal Library", this);
    libraryList_ = new QListWidget();
    libraryList_->setIconSize(QSize(32, 32));
    libraryList_->setDragEnabled(true);
    
    // Create terminal icons
    QMap<QString, QPixmap> terminalIcons = IconFactory::createTerminalIcons();
    
    // Add items with custom icons
    for (auto it = terminalIcons.constBegin(); it != terminalIcons.constEnd(); ++it) {
        QListWidgetItem* item = new QListWidgetItem(QIcon(it.value()), it.key());
        item->setData(Qt::UserRole, it.value());  // Store pixmap for later use
...
void MainWindow::setupRegionManager() {
    regionManagerDock_ = new QDockWidget("Region Manager", this);
    regionManager_ = new RegionManagerWidget(this);
    regionManagerDock_->setWidget(regionManager_);
    addDockWidget(Qt::LeftDockWidgetArea, regionManagerDock_);
    regionManagerDock_->resize(regionManagerDock_->width(), 200);
}
...
void MainWindow::setupLoggingTab() {
    ...
    QGroupBox* generalGroup = new QGroupBox(clientNames_.last());
...
void MainWindow::startQueueProcessing() {
    ...
    connect(logTimer_, &QTimer::timeout, this, &MainWindow::processLogQueue);
    logTimer_->start(100);
    
    progressTimer_ = new QTimer(this);
    connect(progressTimer_, &QTimer::timeout, this, &MainWindow::processProgressQueue);
    progressTimer_->start(100);
}

void MainWindow::processLogQueue() {
    // while (!ApplicationLogger::logQueueEmpty()) {
    //     ApplicationLogger::LogMessage message = ApplicationLogger::getNextLogMessage();

    //     // Add to appropriate log widget in the logging tab
    //     appendLog(message.text, message.widgetIndex, message.isError);

    //     // Also display important messages in the status bar
    //     if (message.isError) {
    //         // Show errors in the backend message area with red color
    //         updateBackendMessage(message.text, "error", 8000);
    //     } else if (message.text.toLower().contains("created") ||
    ...
    //         updateBackendMessage(message.text, "success", 5000);
    //     } else if (message.text.startsWith("Message Sent:")) {
    //         // Show API calls
    //         updateBackendMessage(message.text, "info", 3000);
    //     }
    // }
}
...
void MainWindow::processProgressQueue() {
    ...
    //     ApplicationLogger::ProgressInfo progress = ApplicationLogger::getNextProgressUpdate();
    //     if (progress.clientIndex < progressBars_.size()) {
    //         progressBars_[progress.clientIndex]->setValue(progress.value);
    //     }
    // }
}

void MainWindow::appendLog(const QString& message, int widgetIndex, bool isError) {
...
void MainWindow::setupStatusBar() {
    ...
    statusLabel_->setMaximumWidth(400);  // Limit the width to prevent pushing center section
    ...
    serverInfo["NeTrainSim"] = {
        ...
        {"longDesc", "Open Source Large-Scale Network Train Simulator"}
    };
    serverInfo["ShipNetSim"] = {
        {"shortDesc", "Ships Server"}, 
        {"longDesc", "Open Source Large-Scale Maritime Transport Simulator"}
    };
    serverInfo["INTEGRATION"] = {
        {"shortDesc", "Trucks Server"}, 
        {"longDesc", "Free-ware Large-Scale Traffic Simulator"}
    };
    serverInfo["TerminalSim"] = {
        {"shortDesc", "Terminal Graph Server"}, 
        {"longDesc", "Intermodal Terminal Management System"}
    };
    
    // Add indicators for each server
    QMap<QString, QMap<QString, QVariant>> serverIndicators;
    
    for (auto it = serverInfo.constBegin(); it != serverInfo.constEnd(); ++it) {
        ...
        indicator->setStyleSheet("background-color: #808080; border-radius: 5px;");
        indicator->setToolTip(shortDesc + " - Disconnected");
        ...
        indicatorData["indicator"] = QVariant::fromValue<QLabel*>(indicator);
        indicatorData["label"] = QVariant::fromValue<QLabel*>(label);
    ...
    // Add the center container with a stretch on both sides to keep it centered
    ...
    backendIcon_->setStyleSheet("background-color: #0066cc; border-radius: 5px;");
    ...
    backendReportLabel_->setMaximumWidth(400);  // Limit width
    rightLayout->addWidget(backendReportLabel_);

    // Add the right container
    mainLayout->addWidget(rightContainer);

    // Add our custom widget to take over the entire status bar
    statusBar->addWidget(mainContainer, 1);  // Stretch factor 1 makes it fill the bar
}


RegionCenterPoint* MainWindow::createRegionCenter(const QString& regionName, 
                                                  const QColor& color,
                                                  const QPointF* pos) {
    RegionCenterPoint* centerPoint = new RegionCenterPoint(color);
    connect(centerPoint, &RegionCenterPoint::clicked, this, &MainWindow::updatePropertiesPanel);

    // Add position change connection
    connect(centerPoint, &RegionCenterPoint::positionChanged,
    ...
    Backend::RegionDataController::getInstance()
        .setRegionVariable(regionName,
                           "regionCenterPoint",
                           QVariant::fromValue(centerPoint));
    return centerPoint;
}

void MainWindow::updateGlobalMapForRegion(const QString& regionName) {
    for (QGraphicsItem* item : scene_->items()) {
        TerminalItem* terminalItem = dynamic_cast<TerminalItem*>(item);
        // if (terminalItem && terminalItem->region == regionName) {
        //     ViewController::updateGlobalMapItem(this, terminalItem);
        // }
    }
}

void MainWindow::setConnectionType(const QString& connectionType) {
    currentConnectionType_ = connectionType;

    // Uncheck all other actions
    for (QAction* action : connectionMenu_->actions()) {
        action->setChecked(action->text() == connectionType);
    }

    showStatusBarMessage(QString("Connection type set to: %1").arg(connectionType), 2000);
}
...
void MainWindow::updatePropertiesPanel(QGraphicsItem* item) {
    if (!item) {
        ...
        // propertiesPanel_->displayProperties(item); // TODO
    }
}

void MainWindow::hidePropertiesPanel() {
    // Show map properties only if on main view or global map tabs
    int currentTab = tabWidget_->currentIndex();
    if (currentTab == tabWidget_->indexOf(tabWidget_->widget(0))) {  // Main view tab
...
GraphicsView* MainWindow::getCurrentView() const {
    if (tabWidget_->currentIndex() == 0) {  // Main view tab
        return regionView_;
    } else if (tabWidget_->currentIndex() == 1) {  // Global map tab
...
void MainWindow::handleTabChange(int index) {
    bool isGlobalMap = index == tabWidget_->indexOf(tabWidget_->widget(1));
    bool isLoggingTab = index == tabWidget_->indexOf(tabWidget_->widget(2));
    ...
    // BasicButtonController::toggleMeasureMode(this, false); // TODO

    // Handle tool button visibility based on current tab
    for (auto it = toolsButtonsVisibility_.begin(); it != toolsButtonsVisibility_.end(); ++it) {
    ...
    for (auto it = tabsVisibility_.begin(); it != tabsVisibility_.end(); ++it) {
        int toolbarTabIndex = it.key();
        QList<int> tabIndices = it.value();
        // ToolbarController::getRibbon(this)->setTabVisible(toolbarTabIndex, tabIndices.contains(index)); // TODO
    }

    // Handle dock window visibility
    for (auto it = windowVisibility_.begin(); it != windowVisibility_.end(); ++it) {
        QDockWidget* dockWindow = it.key();
        QMap<QString, QVariant> config = it.value();

        bool isTabAllowed = config["tabs"].toList().contains(index);
        bool isButtonChecked = config["button"].value<QToolButton*>()->isChecked();
        config["button"].value<QToolButton*>()->setEnabled(isTabAllowed);
        dockWindow->setVisible(isTabAllowed && isButtonChecked);
    }

    // Update group visibility
    updateGroupVisibility(toolsGroup_, toolsButtons_);
    updateGroupVisibility(measurementsGroup_, measurementsButtons_);
    updateGroupVisibility(regionGroup_, regionButtons_);
    updateGroupVisibility(networkImportGroup_, networkImportButtons_);
    updateGroupVisibility(navigationGroup_, navigationButtons_);
    updateGroupVisibility(windowsGroup_, windowsButtons_);
    updateGroupVisibility(logsGroup_, logsButtons_);
    updateGroupVisibility(networkToolsGroup_, networkToolsButtons_);
    updateGroupVisibility(simulationToolsGroup_, simulationToolsButtons_);
    updateGroupVisibility(transportationVehiclesGroup_, transportationVehiclesButtons_);
    updateGroupVisibility(visibilityGroup_, visibilityButtons_);

    // Handle shortest paths table visibility
    if (isLoggingTab) {
        // Save current visibility state before hiding
        tableWasVisible_ = shortestPathTableDock_->isVisible();
        shortestPathTableDock_->hide();
    } else if (previousTabIndex_ == tabWidget_->indexOf(tabWidget_->widget(2))) {  // Coming from logging tab
        if (tableWasVisible_) {
...
void MainWindow::updateGroupVisibility(QGroupBox* group, const QList<QToolButton*>& buttons) {
    int currentTab = tabWidget_->currentIndex();
    ...
    for (QToolButton* button : buttons) {
        ...
        if (toolsButtonsVisibility_.contains(button)) {
            if (toolsButtonsVisibility_[button].contains(currentTab)) {
                anyShouldBeVisible = true;
                break;
            }
        } else {
            // If button has no visibility rules, assume it should be visible
            anyShouldBeVisible = true;
            break;
        }
    }
...
void MainWindow::addBackgroundPhoto() {
    try {
        // Open file dialog (using non-native dialog for consistency)
        ...
        if (tabWidget_->currentWidget() == tabWidget_->widget(0)) {  // Main view tab
            // Create a new BackgroundPhotoItem for the main view
            QString currentRegion =
                Backend::RegionDataController::
                getInstance().getCurrentRegion();
            BackgroundPhotoItem* background =
                new BackgroundPhotoItem(pixmap, currentRegion);
            connect(background, &BackgroundPhotoItem::clicked,
                    this, &MainWindow::updatePropertiesPanel);
            connect(background, &BackgroundPhotoItem::positionChanged,
                    [this, background](const QPointF& pos) {
                        if (propertiesPanel_->getCurrentItem() == background) {
                            propertiesPanel_->updatePositionFields(pos);
                        }
                    });

            // Place the photo at the center of the main view
            QPointF viewCenter = regionView_->mapToScene(
                regionView_->viewport()->rect().center()
                );

            double lat, lon;
            std::tie(lat, lon) = regionView_->sceneToWGS84(viewCenter);
            background->getProperties()["Latitude"] = QString::number(lat, 'f', 6);
            background->getProperties()["Longitude"] = QString::number(lon, 'f', 6);
            background->setPos(viewCenter);

            scene_->addItem(background);

            Backend::RegionDataController::getInstance()
                .setRegionVariable(currentRegion,
                                   "backgroundPhotoItem",
                                   QVariant::fromValue(background));
        } else {  // Global map tab
            // Create a new BackgroundPhotoItem for the global map
            BackgroundPhotoItem* background =
                new BackgroundPhotoItem(pixmap, "Global Map");
            connect(background, &BackgroundPhotoItem::clicked,
                    this, &MainWindow::updatePropertiesPanel);
            connect(background, &BackgroundPhotoItem::positionChanged,
                    [this, background](const QPointF& pos) {
            ...
            // Place the photo at the center of the global map view
            QPointF viewCenter = globalMapView_->mapToScene(
                globalMapView_->viewport()->rect().center()
                );
            ...
            Backend::RegionDataController::getInstance()
                .setGlobalVariable("globalBackgroundPhotoItem",
                                   QVariant::fromValue(background));
        }
    } catch (const std::exception& e) {
        qWarning() << "Error in addBackgroundPhoto:" << e.what();
...
void MainWindow::updateAllCoordinates() {
    // TODO
    // for (auto it = regionCenters_.begin(); it != regionCenters_.end(); ++it) {
    //     const QString& regionName = it.key();
    //     RegionCenterPoint* center = it.value();
    //     QPointF pos = center->pos();

    //     double lat, lon;
    //     std::tie(lat, lon) = regionView_->sceneToWGS84(pos);
    //     center->getProperties()["Latitude"] = QString::number(lat, 'f', 6);
    //     center->getProperties()["Longitude"] = QString::number(lon, 'f', 6);
    // }

    // // If properties panel is displaying a region center, update its display
    // if (propertiesPanel_->getCurrentItem() && dynamic_cast<RegionCenterPoint*>(propertiesPanel_->getCurrentItem())) {
...
void MainWindow::showStatusBarError(QString message, int timeout)
{
    statusBar()->showMessage(message, timeout);
}
...
void MainWindow::togglePanMode() {
    ...
    QString newMode = (currentMode == "middle_mouse") ? "ctrl_left" : "middle_mouse";
    ...
    showStatusBarMessage(QString("Panning mode changed to %1").arg(modeText), 2000);
}

void MainWindow::handleTerminalNodeLinking(QGraphicsItem* item) {
    ...
    if (terminalItem) {
        ...
        showStatusBarMessage("Terminal selected. Now select a node to link it to...", 2000);
        return;
    }
    ...
    if (mapPoint && selectedTerminal_) {
        ...
        mapPoint->getProperties()["LinkedTerminal"] = selectedTerminal_->getProperties()["ID"];

        // Update the properties panel if this item is currently selected
        ...
        selectedTerminal_ = nullptr;
        showStatusBarMessage("Terminal linked to node successfully", 2000);

        // Force a redraw of the MapPoint to show the terminal icon
        mapPoint->update();
    } else if (!selectedTerminal_) {
        showStatusBarError("Please select a terminal first", 2000);
    }
}

void MainWindow::handleTerminalNodeUnlinking(QGraphicsItem* item) {
    ...
    MapPoint* mapPoint = dynamic_cast<MapPoint*>(item);
    if (mapPoint) {
        mapPoint->setLinkedTerminal(nullptr);  // Unlink the terminal from the node
        ...
        selectedTerminal_ = nullptr;
        showStatusBarMessage("Terminal unlinked successfully", 2000);
...
void MainWindow::toggleShortestPathsTable(bool show) {
    ...
    //     int totalHeight = savedSplitterSizes_.at(0) + savedSplitterSizes_.at(1);
    ...
    //         centerSplitter->setSizes({int(totalHeight * 0.7), int(totalHeight * 0.3)});
    //     }
    // }
}
...
void MainWindow::updateServerHeartbeat(const QString& serverId, float timestamp) {
    // Delegate to heartbeat controller
    // heartbeatController_->updateServerHeartbeat(serverId, timestamp);
}

void MainWindow::updateBackendMessage(const QString& message, const QString& status, int timeout) {
    ...
    if (status.toLower() == "error" ||
        ...
        backendReportLabel_->setStyleSheet("color: #cc0000; font-weight: bold;");  // Red for errors
        ...
        backendReportLabel_->setStyleSheet("color: #007700;");  // Green for success
    } else {
        backendReportLabel_->setStyleSheet("color: #0066cc;");  // Blue for info
    }
    ...
    if (timeout > 0) {
        QTimer::singleShot(timeout, this, &MainWindow::clearBackendMessage);
    }
}
...
void MainWindow::assignSelectedToCurrentRegion() {
    for (QGraphicsItem* item : scene_->selectedItems()) {
        RegionCenterPoint* centerPoint = dynamic_cast<RegionCenterPoint*>(item);
        if (centerPoint) {
            showStatusBarError("Region center point cannot be assigned to other regions.", 3000);
            return;
        }
        ...
        if (item->type() == QGraphicsItem::UserType + 1) {  // TerminalItem
            TerminalItem* terminal = static_cast<TerminalItem*>(item);
            terminal->setRegion(currentRegion);
        } else if (item->type() == QGraphicsItem::UserType + 2) {  // ConnectionLine
            ConnectionLine* connection = static_cast<ConnectionLine*>(item);
            connection->setRegion(currentRegion);
        } else if (item->type() == QGraphicsItem::UserType + 4) {  // BackgroundPhotoItem
            BackgroundPhotoItem* photo = static_cast<BackgroundPhotoItem*>(item);
            photo->setRegion(currentRegion);
        } else if (item->type() == QGraphicsItem::UserType + 7) {  // MapPoint
            MapPoint* point = static_cast<MapPoint*>(item);
            point->setRegion(currentRegion);
        } else if (item->type() == QGraphicsItem::UserType + 8) {  // MapLine
    ...
    showStatusBarMessage("Selected items assigned to current region.", 2000);
}
...
void MainWindow::keyPressEvent(QKeyEvent* event) {
    ...
    if (event->key() == Qt::Key_Delete) {
        ...
        // Create a copy of the selected items list to safely modify during iteration
        QList<QGraphicsItem*> selectedItems = currentScene->selectedItems();

        // Process ConnectionLabel items first to identify parent lines to remove
        QList<ConnectionLine*> parentLinesToRemove;

        for (auto it = selectedItems.begin(); it != selectedItems.end();) {
            ConnectionLabel* label = dynamic_cast<ConnectionLabel*>(*it);
            if (label) {
                ConnectionLine* parent = dynamic_cast<ConnectionLine*>(label->parentItem());
                if (parent) {
        ...
        // Add parent lines to our selection if not already there
        for (ConnectionLine* parentLine : parentLinesToRemove) {
        ...
        //     TerminalItem* terminal = dynamic_cast<TerminalItem*>(item);
        //     GlobalTerminalItem* globalTerminal = dynamic_cast<GlobalTerminalItem*>(item);

        //     if (terminal || globalTerminal) {
        //         if (currentScene == scene_ && terminal) {
        //             // If in main view, we need to clean up the global map as well
        //             GlobalTerminalItem* globalItem = globalMapItems_.value(terminal, nullptr);

        //             if (globalItem) {
        //                 // Remove all connection lines in the global scene that involve this terminal
        //                 for (QGraphicsItem* connection : globalMapScene_->items()) {
        //                     ConnectionLine* line = dynamic_cast<ConnectionLine*>(connection);
        //                     if (line && (line->startItem() == globalItem || line->endItem() == globalItem)) {
        ...
        //             }
        //         } else if (currentScene == globalMapScene_ && globalTerminal) {
        //             // If in global view, handle GlobalMapTerminalItem
        //             TerminalItem* linkedTerminal = globalTerminal->getLinkedTerminalItem();

        //             // Remove all connection lines in the global scene
        //             for (QGraphicsItem* connection : globalMapScene_->items()) {
        //                 ConnectionLine* line = dynamic_cast<ConnectionLine*>(connection);
        //                 if (line && (line->startItem() == globalTerminal || line->endItem() == globalTerminal)) {
        //                     globalMapScene_->removeItem(line);
        //                 }
        //             }
        ...
        //         // Remove connection lines associated with this terminal in the current scene
        //         for (QGraphicsItem* connection : currentScene->items()) {
        //             ConnectionLine* line = dynamic_cast<ConnectionLine*>(connection);
        //             if (line && (line->startItem() == item || line->endItem() == item)) {
        //                 currentScene->removeItem(line);
        //             }
        //         }

        //         // Remove map point links to this terminal
        //         for (QGraphicsItem* mapPoint : currentScene->items()) {
        //             MapPoint* point = dynamic_cast<MapPoint*>(mapPoint);
        //             if (point && point->getLinkedTerminal() == terminal) {
        ...
        //     else if (dynamic_cast<BackgroundPhotoItem*>(item)) {
        ...
        showStatusBarMessage("Selected items deleted.", 2000);
        event->accept();
    }
