...
/**
 * @brief A custom QGraphicsObject for displaying background photos in regions
 *
 * This class handles background images that can be positioned and scaled on the scene.
 * It maintains its own properties and can be serialized/deserialized for project saving.
 */
...
public:
     ...
     * @param regionName The region this background belongs to
     * @param parent Optional parent item
     */
    BackgroundPhotoItem(const QPixmap& pixmap, const QString& regionName, QGraphicsItem* parent = nullptr);
     ...
     * @param locked True to prevent movement, false to allow movement
     */
    ...
    /**
     * @brief Set the position using WGS84 (geographic) coordinates
     ...
     * @param newProperties A map of property key-value pairs to update
     */
    void updateProperties(const QMap<QString, QVariant>& newProperties);

    /**
     * @brief Update a property and emit propertyChanged signal
    ...
    static BackgroundPhotoItem* fromDict(const QMap<QString, QVariant>& data, QGraphicsItem* parent = nullptr);
...
signals:
    ...
    /**
     * @brief Signal emitted when the item's position changes
     ...
     * @param locked True if the item is now locked, false otherwise
     */
...
private:
    /**
     * @brief Update coordinate properties when position changes
     */
    void updateCoordinates();
    

    QPixmap pixmap;                    ///< The image to display
    QString regionName;                ///< The region this background belongs to
    QMap<QString, QVariant> properties;///< Properties map
    bool locked;                       ///< Whether the item is locked from movement
