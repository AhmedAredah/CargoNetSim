...
/**
 * @brief Graphical representation of a terminal in the freight network
 * 
 * This class represents terminals such as ports, train depots, and truck stops
 * within the CargoNetSim application. Terminals can be dragged, selected, and
...
public:
     ...
     * @param properties Terminal properties as key-value pairs
     * @param region Region the terminal belongs to
     * @param parent Parent graphics item (if any)
     * @param terminalType Type of terminal (e.g., "Origin", "Sea Port Terminal")
     */
     ...
     */
    QString getTerminalType() const { return terminalType; }
    ...
    /**
     * @brief Set class ID counters based on existing terminals
     * 
     * @param allTerminalsById Map of terminal IDs to terminal pointers
     */
    static void setClassIDs(const QMap<int, TerminalItem*>& allTerminalsById);
    
    /**
     * @brief Get a new terminal ID
     * 
     * @param terminalType Optional terminal type for type-specific IDs
     * @return New unique ID
     */
    static int getNewTerminalID(const QString& terminalType = QString());
     ...
     */
    static TerminalItem* fromDict(const QMap<QString, QVariant>& data, 
     ...
     * 
     * Creates a pulsing highlight effect to draw attention to this terminal
     * 
     * @param evenIfHidden Make the terminal temporarily visible if it's hidden
     * @param color Highlight color (semi-transparent red by default)
     */
...
protected:
    /**
     * @brief Define the bounding rectangle for painting and interaction
     * 
    ...
    /**
     * @brief Handle item changes (position, selection, etc.)
     * 
...
private:
    ...
    QMap<QString, QVariant> properties; ///< Terminal properties
    ...
    QPropertyAnimation* animation; ///< Property animation for effects

    // Static ID management
    static int TERMINAL_ID;                  ///< Next general terminal ID
    static QMap<QString, int> TERMINAL_TYPES_IDs; ///< Next ID by terminal type
    
    /**
     * @brief Initialize default properties based on terminal type
     */
