      ...
      properties(properties)
{
    ...
    if (this->properties.isEmpty()) {
        this->properties["x"] = x;
        this->properties["y"] = y;
        this->properties["Network_ID"] = referencedNetworkID;
        this->properties["region"] = region;
    }
    
    // Set higher z-value to ensure points are drawn above lines
    setZValue(10);
...
void MapPoint::setLinkedTerminal(TerminalItem* newTerminal) {
    ...
    if (terminal) {
        properties["LinkedTerminal"] = terminal->getProperties()["ID"];
    } else {
...
void MapPoint::updateProperties(const QMap<QString, QVariant>& newProperties) {
...
void MapPoint::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    if (terminal) {
        // If linked to a terminal, draw terminal icon at reduced opacity
        const QPixmap& pixmap = terminal->getPixmap();
        if (!pixmap.isNull()) {
            painter->setOpacity(0.7); // Slightly transparent to show it's a link
            int pixmapWidth = pixmap.width();
            int pixmapHeight = pixmap.height();
            painter->drawPixmap(
                -pixmapWidth / 2,
...
void MapPoint::contextMenuEvent(QGraphicsSceneContextMenuEvent* event) {
    showContextMenu(event);
    event->accept();
}

void MapPoint::showContextMenu(QGraphicsSceneContextMenuEvent* event) {
    QMenu menu;
    
    // Create sub-actions for terminal creation
    QMenu* createTerminalMenu = menu.addMenu("Create Terminal at Node");
    
    QAction* originAction = menu.addAction("Mark as Origin");
    QAction* destinationAction = menu.addAction("Mark as Destination");
    
    QAction* seaTerminalAction = createTerminalMenu->addAction("Sea Terminal");
    QAction* intermodalTerminalAction = createTerminalMenu->addAction("Intermodal Terminal");
    QAction* trainDepotAction = createTerminalMenu->addAction("Train Depot");
    QAction* parkingAction = createTerminalMenu->addAction("Parking");
...
void MapPoint::createTerminalAtPosition(const QString& terminalType) {
    // This will be implemented to call the ViewController's terminal creation method
    // Since we need to avoid circular dependencies, this should be handled by
    // connecting to the clicked signal and using that to create terminals
    
    // For now, just emit clicked - the actual creation will be handled by MainWindow
    emit clicked(this);
    
    // Note: In a full implementation, this would call a controller method to create
    // the terminal and then link it, but that requires access to MainWindow
}

QMap<QString, QVariant> MapPoint::toDict() const {
    QMap<QString, QVariant> data;
    
    data["referenced_network_ID"] = properties.value("Network_ID");
    data["x"] = x;
    data["y"] = y;
    data["shape"] = shape;
    data["region"] = region;
    ...
    if (terminal) {
        data["terminal_id"] = terminal->getProperties().value("ID");
    }
    
    return data;
}

MapPoint* MapPoint::fromDict(const QMap<QString, QVariant>& data,
                            const QMap<int, TerminalItem*>& terminalsById) {
    // Find linked terminal by ID if terminals dictionary is provided
    TerminalItem* terminal = nullptr;
    if (data.contains("terminal_id") && !terminalsById.isEmpty()) {
    ...
    // Set color
    instance->setColor(QColor(data.value("color", "#000000").toString()));
    
    // Set other properties
    instance->setSelected(data.value("selected", false).toBool());
    instance->setZValue(data.value("z_value", 10).toDouble());
