...
BackgroundPhotoItem::BackgroundPhotoItem(const QPixmap& pixmap, const QString& regionName, QGraphicsItem* parent)
    ...
    , currentOpacity(1.0)
{
    ...
    properties["Type"] = QString("Background - %1").arg(regionName);
...
void BackgroundPhotoItem::updateCoordinates() {
    ...
    // Convert to coordinates - we need to call a method on the main window
    // This would typically be implemented by calling a controller or utility method
    // TODO: we're using a placeholder implementation
    double lat = 0.0, lon = 0.0;
    
    // Using Qt's meta-object system to call the method dynamically
    ...
    // Update properties
    updateProperty("Latitude", QString::number(lat, 'f', 6));
    updateProperty("Longitude", QString::number(lon, 'f', 6));
}

void BackgroundPhotoItem::setFromWGS84(double lat, double lon) {
    // Get scene and view
    QGraphicsScene* graphicsScene = scene();
    if (!graphicsScene || graphicsScene->views().isEmpty()) {
        return;
    ...
    // Convert from coordinates - call a method on the main window
    QPointF scenePos;
...
QRectF BackgroundPhotoItem::boundingRect() const {
    ...
    return QRectF(0, 0, pixmap.width() * scale, pixmap.height() * scale);
}
...
float BackgroundPhotoItem::getScale() const {
    bool ok = false;
    float scale = properties.value("Scale", "1.0").toString().toFloat(&ok);
...
void BackgroundPhotoItem::setScale(float scale) {
    ...
    if (qAbs(scale - getScale()) > 0.001f) {
        updateProperty("Scale", QString::number(scale, 'f', 2));
        updateScale();
    }
}
...
void BackgroundPhotoItem::setOpacity(qreal opacity) {
    ...
    if (qAbs(opacity - currentOpacity) > 0.01) {
        ...
        updateProperty("Opacity", QString::number(opacity, 'f', 2));
        
        // Must call QGraphicsItem's setOpacity which will trigger a redraw
...
void BackgroundPhotoItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    ...
    if (option->state & QStyle::State_Selected) {
        ...
        painter->setPen(pen);
        painter->drawRect(QRectF(0, 0, scaledWidth, scaledHeight));
    }
}

void BackgroundPhotoItem::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    if (!locked) {
        ...
        // Still emit clicked signal when locked, but don't allow movement
        emit clicked(this);
        event->accept();
    }
}

QVariant BackgroundPhotoItem::itemChange(GraphicsItemChange change, const QVariant& value) {
    if (change == ItemPositionChange && scene()) {
        ...
        if (dragOffset != QPointF()) {
            ...
            // If mouse grabber is this item, adjust by cursor position
        ...
        // Update coordinates and notify about position change
        updateCoordinates();
...
void BackgroundPhotoItem::updateProperties(const QMap<QString, QVariant>& newProperties) {
...
void BackgroundPhotoItem::updateProperty(const QString& key, const QVariant& value) {
...
BackgroundPhotoItem* BackgroundPhotoItem::fromDict(const QMap<QString, QVariant>& data, QGraphicsItem* parent) {
    // Convert base64 back to pixmap
    QPixmap pixmap;
    QByteArray imageData = QByteArray::fromBase64(data["image_data"].toString().toLatin1());
    ...
    instance->setLocked(instance->locked); // Will update flags accordingly
