...
void RegionCenterPoint::updateCoordinates(double lat, double lon) {
    ...
    if (properties["Latitude"].toString() != oldLat || 
        ...
        emit propertyChanged("Latitude", properties["Latitude"]);
        emit propertyChanged("Longitude", properties["Longitude"]);
    }
    
    update();
}

void RegionCenterPoint::updateSharedCoordinates(double lat, double lon) {
    QString oldLat = properties["Shared Latitude"].toString();
    QString oldLon = properties["Shared Longitude"].toString();
    
    // Format with 6 decimal places
    properties["Shared Latitude"] = QString::number(lat, 'f', 6);
    properties["Shared Longitude"] = QString::number(lon, 'f', 6);
    
    // Only emit signals if values actually changed
    if (properties["Shared Latitude"].toString() != oldLat || 
        properties["Shared Longitude"].toString() != oldLon) {
        emit sharedCoordinatesChanged(lat, lon);
        emit propertyChanged("Shared Latitude", properties["Shared Latitude"]);
        emit propertyChanged("Shared Longitude", properties["Shared Longitude"]);
    }
...
void RegionCenterPoint::updateProperties(const QMap<QString, QVariant>& newProperties) {
...
void RegionCenterPoint::updateCoordinatesFromPosition() {
    ...
    while (viewObj && !viewObj->inherits("CargoNetSim::GUI::GraphicsView")) {
    ...
    // Then try the call with the correct type name as string
    QPair<double, double> result;

    // First, ensure the type is properly registered with the Qt meta-type system
...
void RegionCenterPoint::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    // Draw outer circle
...
void RegionCenterPoint::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    dragOffset = event->pos();
    emit clicked(this);
    QGraphicsObject::mousePressEvent(event);
}

QVariant RegionCenterPoint::itemChange(GraphicsItemChange change, const QVariant& value) {
    if (change == ItemPositionChange && scene()) {
        // If drag offset is set, adjust position during drag
...
void RegionCenterPoint::hoverEnterEvent(QGraphicsSceneHoverEvent* event) {
    setCursor(QCursor(Qt::PointingHandCursor));
    QGraphicsObject::hoverEnterEvent(event);
}

void RegionCenterPoint::hoverLeaveEvent(QGraphicsSceneHoverEvent* event) {
    unsetCursor();
...
RegionCenterPoint* RegionCenterPoint::fromDict(const QMap<QString, QVariant>& data) {
    ...
    instance->setZValue(data.value("z_value", 2).toDouble());
