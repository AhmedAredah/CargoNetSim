...
DistanceMeasurementTool::DistanceMeasurementTool(GraphicsView* view, QGraphicsItem* parent)
...
void DistanceMeasurementTool::setStartPoint(const QPointF& point)
{
    if (startPoint != point) {
        ...
        // If both points are set, recalculate and emit distance change
        if (!startPoint.isNull() && !endPoint.isNull()) {
            double distance = getDistance();
            emit distanceChanged(distance, getDistanceText());
        }
    }
}

void DistanceMeasurementTool::setEndPoint(const QPointF& point)
{
    if (endPoint != point) {
        prepareGeometryChange();
        endPoint = point;
        ...
        if (!startPoint.isNull() && !endPoint.isNull()) {
            ...
            // Emit measurement completed signal when both points are set
            emit measurementCompleted(startPoint, endPoint, distance);
        }
    }
}
...
QString DistanceMeasurementTool::getDistanceText() const
{
    ...
    if (distance >= 1000.0) {
        return QString("%1 km").arg(distance / 1000.0, 0, 'f', 1);
    } else {
...
bool DistanceMeasurementTool::hasStartPoint() const {
    return !std::isnan(startPoint.x()) && !std::isnan(startPoint.y());
}

bool DistanceMeasurementTool::hasEndPoint() const {
    return !std::isnan(endPoint.x()) && !std::isnan(endPoint.y());
}
...
QRectF DistanceMeasurementTool::boundingRect() const
{
    ...
    qreal padding = (scale != 0.0) ? basePadding / qAbs(scale) : basePadding;
...
void DistanceMeasurementTool::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    if (startPoint.isNull() || endPoint.isNull()) {
        return;
    }
    ...
    pen.setCosmetic(true);  // Line width is constant regardless of zoom
    painter->setPen(pen);
    ...
    QPointF midPoint((startPoint.x() + endPoint.x()) / 2.0, 
                     (startPoint.y() + endPoint.y()) / 2.0);
    
    // Convert scene coordinates to view coordinates for text placement
    QGraphicsView* graphicsView = scene() ? scene()->views().isEmpty() ? nullptr : scene()->views().first() : nullptr;
    if (!graphicsView) {
        return;
    }
    
    QPointF viewPos = graphicsView->mapFromScene(midPoint);
    QPointF screenPos(viewPos.x() - textWidth / 2.0, viewPos.y() - textHeight / 2.0);
    ...
    QRectF textRect(screenPos.x(), screenPos.y(), textWidth, textHeight);
...
double DistanceMeasurementTool::calculateDistance() const
{
    ...
    if (view) {
        ...
        startLat = startLatLon.second;  // Latitude is y in WGS84
        startLon = startLatLon.first;  // Longitude is x in WGS84
        endLat = endLatLon.second;
        endLon = endLatLon.first;
    }
    
    // Calculate distance based on coordinate system
    if (view && view->useProjectedCoords) {
        return calculateProjectedDistance(startLat, startLon, endLat, endLon);
    } else {
        return calculateGeodesicDistance(startLat, startLon, endLat, endLon);
    }
}

double DistanceMeasurementTool::calculateProjectedDistance(double lat1, double lon1, double lat2, double lon2) const
{
    // In projected mode, convert both points using WGS84 World Mercator projection
    QPointF point1, point2;
    
    if (view) {
        point1 = view->convertCoordinates(QPointF(lon1, lat1), "to_projected");
        point2 = view->convertCoordinates(QPointF(lon2, lat2), "to_projected");
        
        // Calculate Euclidean distance in the projected plane (meters)
...
double DistanceMeasurementTool::calculateGeodesicDistance(double lat1, double lon1, double lat2, double lon2) const
{
    // Handle edge cases
    if (qAbs(lat1 - lat2) < 1e-8 && qAbs(lon1 - lon2) < 1e-8) {
        return 0.0;
    }
    ...
    for (int i = 0; i < maxIterations; ++i) {
        ...
        // Calculate sin(sigma)
        sinSigma = qSqrt(
            qPow(cosU2 * sinLambda, 2) +
            qPow(cosU1 * sinU2 - sinU1 * cosU2 * cosLambda, 2)
        );
        ...
        cos2SigmaM = cos2Alpha != 0.0 ? 
            cosSigma - 2.0 * sinU1 * sinU2 / cos2Alpha : 0.0;
        
        double C = f / 16.0 * cos2Alpha * (4.0 + f * (4.0 - 3.0 * cos2Alpha));
        double lambdaPrev = lambda;
        lambda = L + (1.0 - C) * f * sinAlpha * (
            sigma + C * sinSigma * (cos2SigmaM + C * cosSigma * (-1.0 + 2.0 * qPow(cos2SigmaM, 2)))
        );
    ...
    double deltaSigma = B * sinSigma * (
        cos2SigmaM + B / 4.0 * (
            cosSigma * (-1.0 + 2.0 * qPow(cos2SigmaM, 2)) -
            B / 6.0 * cos2SigmaM * (-3.0 + 4.0 * qPow(sinSigma, 2)) * (-3.0 + 4.0 * qPow(cos2SigmaM, 2))
        )
    );
