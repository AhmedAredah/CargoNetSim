...
const QMap<QString, QMap<QString, QVariant>> ConnectionLine::CONNECTION_STYLES = {
    { "Truck", {
      ...
      m_animation(nullptr)
{
    ...
    QColor color = CONNECTION_STYLES.value(m_connectionType).value("color").value<QColor>();
...
void ConnectionLine::setConnectionType(const QString& type) {
    if (m_connectionType != type && CONNECTION_STYLES.contains(type)) {
        m_connectionType = type;
        m_properties["Connection type"] = type;
        
        // Update label
        m_label->setText(type.at(0));
        QColor color = CONNECTION_STYLES.value(type).value("color").value<QColor>();
        m_label->setColor(color);
...
void ConnectionLine::setProperty(const QString& key, const QVariant& value) {
...
void ConnectionLine::createConnections() {
    ...
    if (dynamic_cast<TerminalItem*>(m_startItem)) {
        connect(dynamic_cast<TerminalItem*>(m_startItem), &TerminalItem::positionChanged,
                this, &ConnectionLine::onStartItemPositionChanged);
    } else if (dynamic_cast<GlobalTerminalItem*>(m_startItem)) {
        connect(dynamic_cast<GlobalTerminalItem*>(m_startItem), &GlobalTerminalItem::positionChanged,
                this, &ConnectionLine::onStartItemPositionChanged);
    }
    
    if (dynamic_cast<TerminalItem*>(m_endItem)) {
        connect(dynamic_cast<TerminalItem*>(m_endItem), &TerminalItem::positionChanged,
                this, &ConnectionLine::onEndItemPositionChanged);
    } else if (dynamic_cast<GlobalTerminalItem*>(m_endItem)) {
        connect(dynamic_cast<GlobalTerminalItem*>(m_endItem), &GlobalTerminalItem::positionChanged,
                this, &ConnectionLine::onEndItemPositionChanged);
    }
    
    // Connect label's clicked signal to our clicked signal
    connect(m_label, &ConnectionLabel::clicked, [this]() {
        emit clicked(this);
    });
}
...
void ConnectionLine::onStartItemPositionChanged(const QPointF& newPos) {
    updatePosition(newPos, true);
    emit startPositionChanged(newPos);
}

void ConnectionLine::onEndItemPositionChanged(const QPointF& newPos) {
    updatePosition(newPos, false);
    emit endPositionChanged(newPos);
}

void ConnectionLine::updatePosition(const QPointF& newPos, bool isStart) {
    ...
    if (isStart && !newPos.isNull()) {
        startCenter = newPos + QPointF(m_startItem->boundingRect().width() / 2,
                                    m_startItem->boundingRect().height() / 2);
        endCenter = m_endItem->scenePos() + QPointF(m_endItem->boundingRect().width() / 2,
                                                 m_endItem->boundingRect().height() / 2);
    } else if (!isStart && !newPos.isNull()) {
        startCenter = m_startItem->scenePos() + QPointF(m_startItem->boundingRect().width() / 2,
                                                     m_startItem->boundingRect().height() / 2);
        endCenter = newPos + QPointF(m_endItem->boundingRect().width() / 2,
                                  m_endItem->boundingRect().height() / 2);
    } else {
        startCenter = m_startItem->scenePos() + QPointF(m_startItem->boundingRect().width() / 2,
                                                     m_startItem->boundingRect().height() / 2);
    ...
    if (lineLength > 0) {
        ...
        if (m_connectionType != "Truck") {
            ...
            if (isVertical) {
                ...
                // Use normalized perpendicular vector
    ...
    int padding = std::max(5, CONNECTION_STYLES.value(m_connectionType).value("width").toInt());
    ...
    if (m_connectionType == "Truck") {
        ...
        m_boundingRect = QRectF(
            ...
            std::abs(m_line.x2() - m_line.x1()) + (2 * padding),
            std::abs(m_line.y2() - m_line.y1()) + (2 * padding)
        );
    } else {
        // For curves, include the control point
        QList<QPointF> points = { m_line.p1(), m_ctrlPoint, m_line.p2() };
...
QLineF ConnectionLine::calculateOffsetLine(const QLineF& originalLine) const {
    // Get the offset distance for this connection type
    qreal offset = CONNECTION_STYLES.value(m_connectionType).value("offset").toDouble();
...
QPainterPath ConnectionLine::shape() const {
    // For selection purposes, use a simplified selection area
    // that focuses more on the label and less on the line itself
    QPainterPath path;
    
    // Add the label's shape (with some padding)
    QRectF labelSceneRect = m_label->sceneBoundingRect();
    QRectF labelLocalRect = mapFromScene(labelSceneRect).boundingRect();
    
    // Add padding around the label for easier selection
    int padding = 10;
    labelLocalRect.adjust(-padding, -padding, padding, padding);
    ...
    // Create a narrow rectangle along the line for selection
    if (m_connectionType == "Truck") {
        // For straight line
        qreal angle = m_line.angle() * (M_PI / 180.0);  // Convert to radians
        qreal dx = lineWidth * 0.5 * std::sin(angle);
        qreal dy = lineWidth * 0.5 * std::cos(angle);
        
        QPolygonF poly;
        poly.append(QPointF(start.x() + dx, start.y() - dy));
        poly.append(QPointF(start.x() - dx, start.y() + dy));
        ...
        // Add midpoint with larger radius for better selection of curved lines
...
void ConnectionLine::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    ...
    QMap<QString, QVariant> style = CONNECTION_STYLES.value(m_connectionType);
    ...
    QPen pen(style.value("color").value<QColor>(), 
             ...
             static_cast<Qt::PenStyle>(style.value("style").toInt()));
...
void ConnectionLine::updateProperties(const QMap<QString, QVariant>& newProperties) {
...
void ConnectionLine::flash(bool evenIfHidden, const QColor& color) {
    ...
    QPainterPath path;
    qreal penWidth = CONNECTION_STYLES.value(m_connectionType).value("width").toInt() * 3;
    ...
    QGraphicsPathItem* overlay = new QGraphicsPathItem(path, this);
    overlay->setPen(pen);
    ...
    public:
        AnimationObject(QObject* parent = nullptr) : QObject(parent), _opacity(1.0) {}
        
        qreal opacity() const { return _opacity; }
        void setOpacity(qreal value) { 
            _opacity = value;
            if (_overlay) _overlay->setOpacity(value);
        }
        
        void setOverlay(QGraphicsPathItem* overlay) { _overlay = overlay; }
    ...
    static_cast<AnimationObject*>(m_animObject)->setOverlay(overlay);
...
QMap<QString, QVariant> ConnectionLine::toDict() const {
    ...
    if (TerminalItem* terminal = dynamic_cast<TerminalItem*>(m_startItem)) {
        startItemId = terminal->getProperties().value("ID").toInt();
        startItemType = "TerminalItem";
    } else if (GlobalTerminalItem* globalTerminal = dynamic_cast<GlobalTerminalItem*>(m_startItem)) {
        if (globalTerminal->getLinkedTerminalItem()) {
            startItemId = globalTerminal->getLinkedTerminalItem()->getProperties().value("ID").toInt();
            startItemType = "GlobalTerminalItem";
        }
    }
    
    if (TerminalItem* terminal = dynamic_cast<TerminalItem*>(m_endItem)) {
        endItemId = terminal->getProperties().value("ID").toInt();
        endItemType = "TerminalItem";
    } else if (GlobalTerminalItem* globalTerminal = dynamic_cast<GlobalTerminalItem*>(m_endItem)) {
        if (globalTerminal->getLinkedTerminalItem()) {
            endItemId = globalTerminal->getLinkedTerminalItem()->getProperties().value("ID").toInt();
    ...
    QGraphicsItem* parent)
{
    ...
    if (!startItem) {
        qWarning() << "Start terminal with ID" << startId << "not found";
        return nullptr;
    }
    
    if (!endItem) {
        qWarning() << "End terminal with ID" << endId << "not found";
        return nullptr;
    }
    
    // Handle global items if needed
    if (globalScene && data["start_item_type"].toString() == "GlobalTerminalItem") {
        for (QGraphicsItem* item : globalScene->items()) {
            GlobalTerminalItem* globalItem = dynamic_cast<GlobalTerminalItem*>(item);
            if (globalItem && globalItem->getLinkedTerminalItem() &&
                globalItem->getLinkedTerminalItem()->getProperties().value("ID").toInt() == startId) {
    ...
    if (globalScene && data["end_item_type"].toString() == "GlobalTerminalItem") {
        for (QGraphicsItem* item : globalScene->items()) {
            GlobalTerminalItem* globalItem = dynamic_cast<GlobalTerminalItem*>(item);
            if (globalItem && globalItem->getLinkedTerminalItem() &&
                globalItem->getLinkedTerminalItem()->getProperties().value("ID").toInt() == endId) {
                endItem = globalItem;
                break;
            }
        }
    ...
    connection->setSelected(data.value("selected", false).toBool());
    connection->setZValue(data.value("z_value", 4).toDouble());
    connection->setVisible(data.value("visible", true).toBool());
...
void ConnectionLine::setClassIDs(const QMap<int, ConnectionLine*>& allConnectionsById) {
    ...
    // Find maximum ID
    int maxId = 0;
    for (ConnectionLine* connection : allConnectionsById.values()) {
