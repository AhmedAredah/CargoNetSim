 ...
 * 
 * A global map representation using the same pixmap as TerminalItem but scaled down.
 * Links to the original terminal item and represents it on the global map.
 */
...
public:
     ...
     */
    TerminalItem* getLinkedTerminalItem() const { return linkedTerminalItem; }
    ...
    /**
     * @brief Updates the item's appearance based on properties from linked terminal
     */
    void updateFromLinkedTerminal();

    /**
     * @brief Serializes the GlobalTerminalItem to a dictionary
     * @return Dictionary containing serialized data
     */
    QMap<QString, QVariant> toDict() const;
    
    /**
     * @brief Creates a GlobalTerminalItem from serialized data
     * @param data The serialized data dictionary
     * @param pixmap The pixmap to display
     * @param parent The parent item
     * @return Pointer to new GlobalTerminalItem instance
     */
    static GlobalTerminalItem* fromDict(
...
signals:
    ...
    /**
     * @brief Signal emitted when linked terminal item changes
     * @param oldTerminal Previous terminal
     * @param newTerminal New terminal
     */
    void linkedTerminalChanged(TerminalItem* oldTerminal, TerminalItem* newTerminal);
...
private:
    QPixmap originalPixmap;        ///< The original terminal pixmap
    QPixmap scaledPixmap;          ///< The scaled version for global view
    TerminalItem* linkedTerminalItem; ///< The linked terminal item
};
