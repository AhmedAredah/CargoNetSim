...
TerminalItem::TerminalItem(
    ...
    : QGraphicsObject(parent),
      ...
      wasSelected(false)
{
    // Set a higher Z-value for terminals (will be drawn on top)
    setZValue(11);
    ...
    // Enable ItemSendsGeometryChanges flag to receive position change notifications
    setFlags(
...
void TerminalItem::initializeDefaultProperties() {
    ...
    if (terminalType == "Origin" || terminalType == "Destination") {
        ...
        this->properties["Name"] = QString("%1%2").arg(terminalType).arg(typeId);
        this->properties["Show on Global Map"] = "True";
        this->properties["Available Interfaces"] = interfaces;
    } else {
        this->properties["ID"] = generalId;
        this->properties["Name"] = QString("%1%2").arg(terminalType).arg(typeId);
        ...
        if (terminalType == "Sea Port Terminal" || terminalType == "Intermodal Land Terminal") {
        ...
        if (terminalType == "Sea Port Terminal") {
            ...
            landSide << "Truck" << "Train";
            this->properties["Show on Global Map"] = "False";
    ...
    if (terminalType == "Origin") {
        this->properties["Containers"] = QMap<QString, QVariant>();
    }
}
...
void TerminalItem::updateProperties(const QMap<QString, QVariant>& newProperties) {
...
void TerminalItem::setProperty(const QString& key, const QVariant& value) {
    // Check if property exists and is different
    if (!properties.contains(key) || properties[key] != value) {
...
QVariant TerminalItem::getProperty(const QString& key, const QVariant& defaultValue) const {
...
void TerminalItem::setClassIDs(const QMap<int, TerminalItem*>& allTerminalsById) {
    ...
    // Reset TERMINAL_TYPES_IDs by iterating over all terminals
    TERMINAL_TYPES_IDs.clear();
    for (auto terminal : allTerminalsById) {
        ...
        if (!TERMINAL_TYPES_IDs.contains(terminalType)) {
            ...
            TERMINAL_TYPES_IDs[terminalType] = qMax(TERMINAL_TYPES_IDs[terminalType], terminalId);
        }
    }
    
    // Ensure the next ID for each terminal type starts from max_ID + 1
    for (auto it = TERMINAL_TYPES_IDs.begin(); it != TERMINAL_TYPES_IDs.end(); ++it) {
        *it = *it + 1;
    }
}

int TerminalItem::getNewTerminalID(const QString& terminalType) {
...
void TerminalItem::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    // Store the initial click position relative to the item's origin
...
QVariant TerminalItem::itemChange(GraphicsItemChange change, const QVariant& value) {
    if (change == ItemPositionChange && scene()) {
        // If this is a position change and we have a drag offset, adjust the position
        if (dragOffset != QPointF()) {
            ...
            if (scene()->mouseGrabberItem() == this) {
                ...
                // Adjust position to keep item under the mouse at the right offset
                return mousePos - dragOffset;
            }
        }
    } else if (change == ItemPositionHasChanged && scene()) {
        // Emit position changed signal when position has been changed
...
void TerminalItem::flash(bool evenIfHidden, const QColor& color) {
    ...
    public:
        ...
        void setOpacity(qreal opacity) {
            _opacity = opacity;
            if (_rect) _rect->setOpacity(opacity);
        }
    ...
    static_cast<AnimationObject*>(animObject)->setRect(rect);
    
    // Create and configure animation
    animation = new QPropertyAnimation(animObject, "opacity", this);
...
TerminalItem* TerminalItem::fromDict(
    const QMap<QString, QVariant>& data,
    const QPixmap& pixmap,
    QGraphicsItem* parent)
{
    // Create new instance with essential data
    TerminalItem* instance = new TerminalItem(
        pixmap,
    ...
    // Set position
    QMap<QString, QVariant> posMap = data["position"].toMap();
    QPointF pos(
        posMap.value("x", 0).toDouble(), 
        posMap.value("y", 0).toDouble()
    );
    instance->setPos(pos);
    
    // Set other properties
    instance->setSelected(data.value("selected", false).toBool());
    instance->setVisible(data.value("visible", true).toBool());
    instance->setZValue(data.value("z_value", 11).toDouble());
