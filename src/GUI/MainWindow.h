 ...
 * 
 * The MainWindow class is implemented as a singleton and manages the
 * entire application UI, including views, scenes, docks, and toolbars.
 */
class MainWindow : public CustomMainWindow {
    Q_OBJECT

    // Declare the ToolbarController and BasicButtonController
    // classes as a friend
...
public:
    ...
    /**
     * @brief Updates the properties panel with the selected item's properties
     ...
     * @return True if global view is active, false otherwise
     */
    bool isGlobalViewActive() const;
    
    /**
     * @brief Checks if the region view is active
     * @return True if region view is active, false otherwise
     */
    ...
    /**
     * @brief Updates coordinates of all region centers and terminals
     */
    void updateAllCoordinates();

    void showStatusBarMessage(QString message, int timeout = 0);

    void showStatusBarError(QString message, int timeout = 0);
...
public slots:
     ...
     */
    void updateServerHeartbeat(const QString& serverId, float timestamp);
     ...
     */
    void updateBackendMessage(const QString& message, const QString& status = "info", int timeout = 5000);
...
private:
     ...
     * @param pos The position of the region center (optional)
     * @return Pointer to the created RegionCenterPoint
     */
    RegionCenterPoint* createRegionCenter(const QString& regionName, const QColor& color, const QPointF* pos = nullptr);
    
    /**
     * @brief Updates global map items for a region
     * @param regionName The name of the region to update
     */
    void updateGlobalMapForRegion(const QString& regionName);
    ...
    /**
     * @brief Appends a log message to the appropriate widget
     * @param message The message to append
     * @param widgetIndex The index of the widget to append to
     * @param isError True if the message is an error
     */
    void appendLog(const QString& message, int widgetIndex, bool isError);
     ...
     */
    void updateGroupVisibility(QGroupBox* group, const QList<QToolButton*>& buttons);
...
protected:
    ...
    TerminalItem* selectedTerminal_; // For linking terminals to nodes
    
    // State management
    QMap<QToolButton*, QList<int>> toolsButtonsVisibility_;
    QMap<int, QList<int>> tabsVisibility_;
    QMap<QDockWidget*, QMap<QString, QVariant>> windowVisibility_;
    QMap<NetworkManagerDialog*, QMap<QString, QVariant>> networkManagerVisibility_;
