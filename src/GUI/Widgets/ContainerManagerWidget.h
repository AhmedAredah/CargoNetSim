 ...
 * 
 * ContainerManagerWidget provides a dialog for viewing, adding, editing, 
 * and deleting containers associated with a terminal.
 */
...
public:
     ...
     */
    explicit ContainerManagerWidget(const QMap<QString, QVariant>& containers, QWidget* parent = nullptr);
    ...
    /**
     * @brief Convert custom ContainerSize enum to containerlib size enum
     * @param customSize Size value from ContainerSize enum
     * @return Corresponding containerlib size value
     */
    static int convertToContainerlibSize(ContainerSize customSize);
    
    /**
     * @brief Convert string representation of container size to enum value
     * @param sizeStr String representation of container size
     * @return Corresponding ContainerSize enum value
     */
    static ContainerSize containerSizeFromString(const QString& sizeStr);
    
    /**
     * @brief Convert ContainerSize enum to string representation
     * @param size ContainerSize enum value
     * @return String representation of container size
     */
    static QString containerSizeToString(ContainerSize size);
...
private slots:
    ...
    /**
     * @brief Generate multiple containers with same properties
     */
    void generateContainers();

private:
    /**
     * @brief Create a combo box for container size selection
     * @param currentSize Current container size
     * @return Configured combo box
     */
    QComboBox* createSizeComboBox(const QString& currentSize);
    ...
    QTableWidget* containerList;          ///< Table widget for container list
...
public:
     ...
     * @param parent Parent widget
     */
    explicit ContainerEditDialog(const QMap<QString, QVariant>& containerData = QMap<QString, QVariant>(), 
...
private:
     ...
     */
    void loadProperties(const QMap<QString, QVariant>& props);

    QTableWidget* propTable;              ///< Table widget for properties
};
...
public:
     ...
     * @param parent Parent widget
     */
    explicit GenerateContainersDialog(QWidget* parent = nullptr);
    
    /**
     * @brief Get the generation parameters
     * @return Map containing count and size for container generation
     */
