...
void TrainManagerDialog::initUI()
{
    ...
    // Load trains action
    QAction* loadAction = new QAction(tr("Load Trains"), this);
    loadAction->setIcon(QIcon(IconFactory::createImportTrainsIcon()));
    loadAction->setToolTip(tr("Load trains from DAT file"));
    
    m_loadButton = new QToolButton();
    m_loadButton->setDefaultAction(loadAction);
    m_loadButton->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
    m_loadButton->setText(tr("Load\nTrains"));
    connect(m_loadButton, &QToolButton::clicked,
            this, &TrainManagerDialog::loadTrains);
    toolbar->addWidget(m_loadButton);

    // Delete train action
    QAction* deleteAction = new QAction(tr("Delete Train"), this);
    deleteAction->setIcon(QIcon(IconFactory::createDeleteTrainIcon()));
    ...
    connect(m_deleteButton, &QToolButton::clicked,
            this, &TrainManagerDialog::deleteTrain);
    ...
    m_table->setHorizontalHeaderLabels({tr("Train ID"),
                                        tr("Locomotives"),
                                        tr("Cars")});
    ...
    // Connect selection change to update details and enable delete button
...
void TrainManagerDialog::setTrains(const QVector<Backend::Train *> trains)
{
    m_trains = trains;
    updateTable();
...
QVector<Backend::Train *> TrainManagerDialog::getTrains() const
{
    return m_trains;
}
    ...
    try {
        ...
        if (newTrains.isEmpty()) {
            QMessageBox::warning(
                this, 
                tr("Warning"), 
                tr("No trains were found in the selected file."));
        ...
        QMessageBox::information(
            this, 
            tr("Success"), 
            tr("Successfully loaded %1 trains.").arg(newTrains.size()));
...
void TrainManagerDialog::deleteTrain()
{
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        this,
        tr("Confirm Deletion"),
        tr("Are you sure you want to delete train '%1'?").arg(trainId),
...
void TrainManagerDialog::updateTable()
{
    ...
    for (const auto& train : m_trains) {
        ...
        QTableWidgetItem* locoItem = new QTableWidgetItem(locoStr);
        ...
        QTableWidgetItem* carsItem = new QTableWidgetItem(carsStr);
...
QString
TrainManagerDialog::formatTrainDetails(const Backend::Train* train) const
{
    ...
    if (pathNodes.isEmpty()) {
        ...
        for (const auto& nodeId : pathNodes) {
            details += QString("<li>Node %1</li>").arg(nodeId);
        }
        details += "</ul>";
    }
