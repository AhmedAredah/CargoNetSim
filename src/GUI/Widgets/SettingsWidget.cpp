...
void SettingsWidget::initUI() {
    ...
    // Create a QScrollArea that will contain all the settings controls
    ...
    simLayout->addRow(tr("Number of Shortest Paths:"), shortestPathsSpin);
    ...
    fuelTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::ResizeToContents);
    fuelTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::ResizeToContents);
...
void SettingsWidget::updateFuelTable() {
    ...
    for(auto it = fuelTypes.begin(); it != fuelTypes.end(); ++it, ++row) {
        ...
        // Fuel type name
        QTableWidgetItem* nameItem = new QTableWidgetItem(fuelType);
        ...
        calorificSpin->setSuffix(tr(" kWh/") + data["unit"].toString());
        ...
        carbonSpin->setSuffix(tr(" kg CO₂/") + data["unit"].toString());
...
void SettingsWidget::updateFuelData(const QString& fuelType, const QString& key, const QVariant& value) {
    if (fuelTypes.contains(fuelType)) {
        fuelTypes[fuelType][key] = value;
        
        // If unit changes, update the suffixes in the table
        if (key == "unit") {
            for (int row = 0; row < fuelTable->rowCount(); ++row) {
                QTableWidgetItem* nameItem = fuelTable->item(row, 0);
                if (nameItem && nameItem->text() == fuelType) {
                    QDoubleSpinBox* costSpin = qobject_cast<QDoubleSpinBox*>(fuelTable->cellWidget(row, 1));
                    QDoubleSpinBox* calorificSpin = qobject_cast<QDoubleSpinBox*>(fuelTable->cellWidget(row, 2));
                    QDoubleSpinBox* carbonSpin = qobject_cast<QDoubleSpinBox*>(fuelTable->cellWidget(row, 3));
                    
                    QString unitStr = value.toString();
                    if (unitStr == "kg") {
                        costSpin->setSuffix(tr(" per ton"));
                    } else {
                        costSpin->setSuffix(tr(" per ") + unitStr);
                    }
                    
                    calorificSpin->setSuffix(tr(" kWh/") + unitStr);
                    carbonSpin->setSuffix(tr(" kg CO₂/") + unitStr);
                    break;
                }
            }
        }
...
void SettingsWidget::addFuelType() {
    ...
    QLabel* nameLabel = new QLabel(tr("Fuel Type Name:"), this);
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (!fuelName.isEmpty() && !fuelTypes.contains(fuelName)) {
...
void SettingsWidget::updateFuelTypeDropdowns() {
    ...
    QString shipFuel = shipFuelType->count() > 0 ? shipFuelType->currentText() : "HFO";
    QString trainFuel = trainFuelType->count() > 0 ? trainFuelType->currentText() : "diesel_1";
    QString truckFuel = truckFuelType->count() > 0 ? truckFuelType->currentText() : "diesel_2";
    ...
    for (auto it = fuelTypes.begin(); it != fuelTypes.end(); ++it) {
        const QString& fuelType = it.key();
    ...
    if (shipIdx >= 0) shipFuelType->setCurrentIndex(shipIdx);
    if (trainIdx >= 0) trainFuelType->setCurrentIndex(trainIdx);
    if (truckIdx >= 0) truckFuelType->setCurrentIndex(truckIdx);
}
...
bool SettingsWidget::loadSettings() {
    try {
        ...
        simulation["time_value_of_money"] = timeValueOfMoneySpin->value();
        simulation["shortest_paths"] = shortestPathsSpin->value();
        settings["simulation"] = simulation;
        
        // Set simulation values from settings
        timeStepSpin->setValue(settings["simulation"].toMap()["time_step"].toInt());
        timeValueOfMoneySpin->setValue(settings["simulation"].toMap()["time_value_of_money"].toDouble());
        shortestPathsSpin->setValue(settings["simulation"].toMap()["shortest_paths"].toInt());
        
        // Set carbon taxes
        QMap<QString, QVariant> carbonTaxes;
        carbonTaxes["rate"] = carbonRateSpin->value();
        carbonTaxes["ship_multiplier"] = shipMultiplierSpin->value();
        carbonTaxes["truck_multiplier"] = truckMultiplierSpin->value();
        carbonTaxes["train_multiplier"] = trainMultiplierSpin->value();
        settings["carbon_taxes"] = carbonTaxes;
        
        carbonRateSpin->setValue(settings["carbon_taxes"].toMap()["rate"].toDouble());
        shipMultiplierSpin->setValue(settings["carbon_taxes"].toMap()["ship_multiplier"].toDouble());
        truckMultiplierSpin->setValue(settings["carbon_taxes"].toMap()["truck_multiplier"].toDouble());
        trainMultiplierSpin->setValue(settings["carbon_taxes"].toMap()["train_multiplier"].toDouble());
        ...
        shipSettings["average_fuel_consumption"] = shipFuelSpin->value();
        shipSettings["average_container_number"] = shipContainers->value();
        ...
        trainSettings["average_speed"] = trainSpeedSpin->value();
        trainSettings["use_network"] = trainUseNetwork->isChecked();
        trainSettings["average_fuel_consumption"] = trainFuelSpin->value();
        trainSettings["average_container_number"] = trainContainers->value();
        trainSettings["risk_factor"] = trainRiskSpin->value();
        trainSettings["fuel_type"] = trainFuelType->currentText();
        transportModes["train"] = trainSettings;

        // Truck settings
        QMap<QString, QVariant> truckSettings;
        truckSettings["average_speed"] = truckSpeedSpin->value();
        truckSettings["use_network"] = truckUseNetwork->isChecked();
        truckSettings["average_fuel_consumption"] = truckFuelSpin->value();
        truckSettings["average_container_number"] = truckContainers->value();
        truckSettings["risk_factor"] = truckRiskSpin->value();
        truckSettings["fuel_type"] = truckFuelType->currentText();
        transportModes["truck"] = truckSettings;

        settings["transport_modes"] = transportModes;

        // Apply transport mode settings
        shipSpeedSpin->setValue(settings["transport_modes"]
                                    .toMap()["ship"]
                                    .toMap()["average_speed"].toDouble());
        shipFuelSpin->setValue(settings["transport_modes"]
                                   .toMap()["ship"]
                                   .toMap()["average_fuel_consumption"]
                                   .toDouble());
        shipContainers->setValue(settings["transport_modes"]
                                     .toMap()["ship"]
                                     .toMap()["average_container_number"]
                                     .toInt());
        shipRiskSpin->setValue(settings["transport_modes"]
                                   .toMap()["ship"]
                                   .toMap()["risk_factor"].toDouble());

        trainSpeedSpin->setValue(settings["transport_modes"]
                                     .toMap()["train"]
                                     .toMap()["average_speed"].toDouble());
        trainUseNetwork->setChecked(settings["transport_modes"]
                                        .toMap()["train"]
                                        .toMap()["use_network"].toBool());
        trainFuelSpin->setValue(settings["transport_modes"]
                                    .toMap()["train"]
                                    .toMap()["average_fuel_consumption"]
                                    .toDouble());
        trainContainers->setValue(settings["transport_modes"]
                                      .toMap()["train"]
                                      .toMap()["average_container_number"]
                                      .toInt());
        ...
        truckSpeedSpin->setValue(settings["transport_modes"]
                                     .toMap()["truck"]
                                     .toMap()["average_speed"]
                                     .toDouble());
        truckUseNetwork->setChecked(settings["transport_modes"]
                                        .toMap()["truck"]
                                        .toMap()["use_network"]
                                        .toBool());
        truckFuelSpin->setValue(settings["transport_modes"]
                                    .toMap()["truck"]
                                    .toMap()["average_fuel_consumption"]
                                    .toDouble());
        truckContainers->setValue(settings["transport_modes"]
                                      .toMap()["truck"]
                                      .toMap()["average_container_number"]
                                      .toInt());
        truckRiskSpin->setValue(settings["transport_modes"]
                                    .toMap()["truck"].toMap()["risk_factor"]
                                    .toDouble());

        // Set fuel types from settings if available
        QString shipFuelTypeName = settings["transport_modes"]
                                       .toMap()["ship"]
                                       .toMap()["fuel_type"].toString();
        if(!shipFuelTypeName.isEmpty()) {
            int idx = shipFuelType->findText(shipFuelTypeName);
            if (idx >= 0) {
        ...
        QString trainFuelTypeName =
            settings["transport_modes"].toMap()["train"]
                .toMap()["fuel_type"].toString();
        if(!trainFuelTypeName.isEmpty()) {
            int idx = trainFuelType->findText(trainFuelTypeName);
            if (idx >= 0) {
        ...
        QString truckFuelTypeName =
            settings["transport_modes"].toMap()["truck"]
                .toMap()["fuel_type"].toString();
        if(!truckFuelTypeName.isEmpty()) {
            int idx = truckFuelType->findText(truckFuelTypeName);
            if (idx >= 0) {
        ...
        qWarning() << "Failed to load settings from config file:" << e.what();
        return false;
    }
}
...
void SettingsWidget::applySettings() {
    ...
    for(auto it = fuelTypes.begin(); it != fuelTypes.end(); ++it) {
        ...
        fuelCarbonContent[fuelType] = data["carbon_content"];
    }
    ...
    // TODO: Show a message in the status bar if the parent is a main window
    ...
    //     mainWindow->findChild<QStatusBar*>()->showMessage(tr("Settings applied."), 3000);
    // }
}
...
void SettingsWidget::showEnergyCalculator(const QString& mode) {
    QDialog dialog(this);
    dialog.setWindowTitle(tr("Energy Consumption Calculator"));
    QVBoxLayout* layout = new QVBoxLayout(&dialog);

    // Get current fuel consumption from settings
    double currentFuelConsumption = 0.0;
    if (!settings.isEmpty() && settings.contains("transport_modes")) {
        auto transportModes = settings["transport_modes"].toMap();
        if (transportModes.contains(mode)) {
            currentFuelConsumption =
                transportModes[mode].toMap()["average_fuel_consumption"]
                    .toDouble();
        }
    }
...
SettingsWidget::createDwellTimeParameters(
    ...
    if (method == "gamma") {
        // Shape parameter
        QLineEdit* shape =
            new QLineEdit(currentParams.value("shape", "2.0").toString());
        paramFields["shape"] = shape;
        paramLayout->addRow(tr("Shape (k):"), shape);

        // Scale parameter
        QLineEdit* scale =
            new QLineEdit(currentParams.value("scale", "1440").toString());
        ...
        // Scale parameter
        QLineEdit* scale =
            new QLineEdit(currentParams.value("scale", "2880").toString());
        paramFields["scale"] = scale;
        ...
        // Mean parameter
        QLineEdit* mean =
            new QLineEdit(currentParams.value("mean", "2880").toString());
        paramFields["mean"] = mean;
        paramLayout->addRow(tr("Mean (minutes):"), mean);

        // Standard deviation parameter
        QLineEdit* stdDev =
            new QLineEdit(currentParams.value("std_dev", "720").toString());
        ...
        QLineEdit* mean =
            new QLineEdit(currentParams.value("mean", "3.45").toString());
        paramFields["mean"] = mean;
        paramLayout->addRow(tr("Mean (log-scale):"), mean);

        // Sigma parameter
        QLineEdit* sigma =
            new QLineEdit(currentParams.value("sigma", "0.25").toString());
...
void SettingsWidget::onDwellMethodChanged(const QString& method,
    ...
    QPair<QFormLayout*, QMap<QString, QWidget*>> paramData =
        createDwellTimeParameters(method);
    QFormLayout* paramLayout = paramData.first;
    QMap<QString, QWidget*> paramFields = paramData.second;

    // Set parent for widgets created in createDwellTimeParameters
    for (auto it = paramFields.begin(); it != paramFields.end(); ++it) {
        QWidget* widget = it.value();
        widget->setParent(dwellGroup); // FIXED: Set correct parent for widgets
    }
    ...
    for (auto it = paramFields.begin(); it != paramFields.end(); ++it) {
        ...
        if (QLineEdit* lineEdit = qobject_cast<QLineEdit*>(widget)) {
            settings[QString("dwell_time.parameters.%1").arg(paramName)] =
                lineEdit->text();
        }
...
void SettingsWidget::addNestedPropertiesSection(
    ...
    QMap<QString, QVariant> properties = item[propertiesKey].toMap();

    for (auto it = properties.begin(); it != properties.end(); ++it) {
        const QString& subkey = it.key();
        const QVariant& subvalue = it.value();

        QLineEdit* lineEdit = new QLineEdit(subvalue.toString(), group);

        if (propertyConfigs.contains(subkey)) {
            QString label = propertyConfigs[subkey].first;
            QValidator* validator = propertyConfigs[subkey].second;

            if (validator) {
                if (QDoubleValidator* doubleValidator =
                    qobject_cast<QDoubleValidator*>(validator)) {
                    doubleValidator->setBottom(0.0); // Ensure non-negative values
                }
