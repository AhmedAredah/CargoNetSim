...
ErrorHandlers::ErrorHandlers() : QObject(nullptr) {
...
void ErrorHandlers::installExceptionHandlers() {
    ...
    std::set_terminate([]() {
        try {
            ...
            // Last resort
            QString errorMsg = "Unhandled exception of unknown type";
            getInstance().errorOccurred(errorMsg, 3); // Fatal
            writeToErrorLog(errorMsg);
            std::cerr << errorMsg.toStdString() << std::endl;
        }
...
void SafeRunnable::run() {
    try {
        ...
        QString errorMsg = QString("Exception in thread: %1").arg(e.what());
        ErrorHandlers::getInstance().errorOccurred(errorMsg, 2); // Error
