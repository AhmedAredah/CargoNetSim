...
QMap<QString, QPixmap> createTerminalIcons() {
    ...
    // --- Origin (Modern circular design with subtle gradient) ---
    {
    ...
    // --- Intermodal Terminal (Modern container transfer design) ---
    {
        QPixmap pixmap(32, 32);
        pixmap.fill(Qt::transparent);
        QPainter painter(&pixmap);
    ...
    // --- Truck Parking (Modern parking design) ---
    {
        ...
        painter.drawText(QRect(11, 24, 12, 12), Qt::AlignCenter, "P");
        painter.end();
...
QPixmap createAssignSelectedToCurrentRegionPixmap(int size) {
    ...
    QRect region_rect(size / 2, size / 2, size / 2 - 4, size / 2 - 4);
    ...
    double arrow_size = 4;
    double angle = std::atan2(region_center.y() - selection_center.y(),
                              region_center.x() - selection_center.x());
    QPointF arrow_p1(region_center.x() - arrow_size * std::cos(angle - M_PI / 6),
                     region_center.y() - arrow_size * std::sin(angle - M_PI / 6));
    QPointF arrow_p2(region_center.x() - arrow_size * std::cos(angle + M_PI / 6),
                     region_center.y() - arrow_size * std::sin(angle + M_PI / 6));
    QPolygonF arrow_head;
    arrow_head << QPointF(region_center) << arrow_p1 << arrow_p2;
...
QPixmap createSetBackgroundColorPixmap(int size) {
    ...
    QLinearGradient gradient(QPointF(rect.topLeft()), QPointF(rect.bottomRight()));
...
QPixmap createMeasureDistancePixmap(int size) {
    ...
    for (int x = 6; x < size - 4; x += 6) {
        if (x % 12 == 0)
            painter.drawLine(x, size / 2 - 6, x, size / 2 + 6);
        else
            painter.drawLine(x, size / 2 - 4, x, size / 2 + 4);
    }
    painter.end();
    return pixmap;
}
...
QPixmap createClearMeasurementsPixmap(int size) {
    ...
    for (int x = 10; x < size - 8; x += 5) {
        if (x % 10 == 0)
            painter.drawLine(x, ruler_y - 4, x, ruler_y + 4);
        else
...
QPixmap createPropertiesIcon(int size) {
    ...
    QPainter painter(&pixmap);
    painter.setRenderHint(QPainter::Antialiasing);
    QPen pen(QColor("#4CAF50"), std::max(1, int(size * 0.08)));
    painter.setPen(pen);
    int rect_height = int(size * 0.15);
    for (int i = 0; i < 3; ++i) {
        int y_offset = int((size * 0.2) + (i * rect_height * 1.5));
        painter.drawRect(int(size * 0.2), y_offset, int(size * 0.6), rect_height);
    }
    painter.end();
    return pixmap;
}
...
QPixmap createFreightTerminalLibraryIcon(int size) {
    ...
    const QStringList colors = { "#FF9800", "#F57C00", "#FFA726" };
    for (int i = 0; i < colors.size(); ++i) {
        painter.setBrush(QBrush(QColor(colors[i])));
        painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.03))));
        painter.drawRect(int(size * 0.2),
                         int(size * 0.2 + i * size * 0.2),
                         int(size * 0.6),
                         int(size * 0.15));
    }
    painter.setPen(QPen(QColor("#000000"), std::max(1, int(size * 0.04))));
    int crane_x = int(size * 0.5);
    painter.drawLine(crane_x, int(size * 0.05), crane_x, int(size * 0.2));
    painter.end();
    return pixmap;
}
...
QPixmap createRegionManagerIcon(int size) {
    ...
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.04))));
    int hub_x = int(size * 0.45);
    int hub_y = int(size * 0.1);
    painter.drawEllipse(hub_x, hub_y, int(size * 0.12), int(size * 0.12));
    ...
    for (const QPoint &pt : nodes) {
        ...
        // Draw each node as an ellipse (using half the intended diameter)
        painter.drawEllipse(pt, int(size * 0.06), int(size * 0.06));
    }

    QPen pen(link_color, std::max(1, int(size * 0.03)));
    painter.setPen(pen);
    for (const QPoint &pt : nodes) {
        painter.drawLine(hub_x + int(size * 0.06), hub_y + int(size * 0.06),
                         pt.x() + int(size * 0.06), pt.y() + int(size * 0.06));
    }
...
QPixmap createSimulationSettingsIcon(int size) {
    ...
    QVector<int> x_positions = { int(size * 0.2), int(size * 0.45), int(size * 0.7) };

    for (int i = 0; i < 3; ++i) {
        painter.setBrush(QBrush(QColor("#9C27B0"))); // Purple color
        painter.setPen(Qt::NoPen);
        int rect_x = x_positions[i];
        int rect_y = int(size * (1 - heights[i]));
        int rect_height = int(size * heights[i]);
        painter.drawRect(rect_x, rect_y, bar_width, rect_height);
    }
...
QPixmap createShowHideGridIcon(int size) {
    ...
    QPen pen(QColor("#B0BEC5"), std::max(1, int(size * 0.03)));
    painter.setPen(pen);
    ...
    for (int i = 1; i <= num_lines; ++i) {
        int x = i * spacing;
        painter.drawLine(x, int(size * 0.1), x, int(size * 0.9));
        int y = i * spacing;
        painter.drawLine(int(size * 0.1), y, int(size * 0.9), y);
    }
    ...
    painter.drawEllipse(eye_x + eye_width / 3, eye_y + eye_height / 3,
...
QPixmap createFreightTrainIcon(int size) {
    ...
    QLinearGradient gradient(size * 0.1, size * 0.3,
                             size * 0.1, size * 0.3 + train_body_height);
    gradient.setColorAt(0, QColor("#F44336")); // Lighter red
    gradient.setColorAt(1, QColor("#B71C1C"));   // Darker red
    painter.setBrush(gradient);
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
    painter.drawRoundedRect(int(size * 0.1), int(size * 0.3),
                            int(size * 0.25), train_body_height, 8, 8);

    QLinearGradient window_gradient(size * 0.15, size * 0.35,
                                    size * 0.15, size * 0.45);
    window_gradient.setColorAt(0, QColor("#FFFFFF"));
    window_gradient.setColorAt(1, QColor("#B0BEC5"));
    painter.setBrush(window_gradient);
    painter.drawRoundedRect(int(size * 0.15), int(size * 0.35),
                            int(size * 0.1), int(size * 0.1), 5, 5);

    QRadialGradient radial_gradient(size * 0.18, size * 0.45, size * 0.04);
    ...
    for (int i = 0; i < container_colors.size(); ++i) {
        ...
        QLinearGradient container_gradient(x_offset, size * 0.3,
                                           x_offset, size * 0.3 + train_body_height);
        container_gradient.setColorAt(0, QColor(container_colors[i].first));
        container_gradient.setColorAt(1, QColor(container_colors[i].second));
        painter.setBrush(container_gradient);
        painter.drawRoundedRect(x_offset, int(size * 0.32),
                                int(size * 0.25), int(train_body_height * 0.8), 6, 6);
        painter.setPen(QPen(QColor(container_colors[i].second),
                            std::max(1, int(size * 0.01))));
        for (int y = int(size * 0.34); y < int(size * 0.6); y += int(size * 0.06)) {
            painter.drawLine(x_offset + 2, y,
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        wheel_gradient.setCenter(x_offset + wheel_radius, track_y + wheel_radius);
        wheel_gradient.setRadius(wheel_radius);
        painter.setBrush(wheel_gradient);
        painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
        painter.drawEllipse(x_offset, track_y, wheel_radius * 2, wheel_radius * 2);
        painter.setBrush(QBrush(QColor("#757575")));
        painter.drawEllipse(x_offset + int(wheel_radius * 0.6),
                            track_y + int(wheel_radius * 0.6),
                            int(wheel_radius * 0.8), int(wheel_radius * 0.8));
    }

    QColor track_color("#E0E0E0");
    QColor tie_color("#8D6E63");
    painter.setPen(QPen(track_color, std::max(2, int(size * 0.02))));
    int y_offset = track_y + wheel_radius;
    painter.drawLine(int(size * 0.05), y_offset, int(size * 0.95), y_offset);
    painter.drawLine(int(size * 0.05), y_offset + 4, int(size * 0.95), y_offset + 4);
    QLinearGradient tie_gradient(0, y_offset, 0, y_offset + size * 0.04);
...
QPixmap createFreightTruckIcon(int size) {
    ...
    QLinearGradient trailer_gradient(size * 0.2, size * 0.35,
                                     size * 0.2, size * 0.35 + truck_body_height);
    trailer_gradient.setColorAt(0, QColor("#1E88E5"));
    trailer_gradient.setColorAt(1, QColor("#1565C0"));
    painter.setBrush(trailer_gradient);
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
    painter.drawRoundedRect(int(size * 0.2), int(size * 0.35),
                            truck_body_width, truck_body_height, 6, 6);

    painter.setPen(QPen(QColor("#0D47A1"), std::max(1, int(size * 0.01))));
    for (int y = int(size * 0.38); y < int(size * 0.62); y += int(size * 0.06)) {
        painter.drawLine(int(size * 0.22), y, int(size * 0.83), y);
    }

    QLinearGradient cab_gradient(size * 0.1, size * 0.4,
                                 size * 0.1, size * 0.4 + cab_height);
    cab_gradient.setColorAt(0, QColor("#F44336"));
    cab_gradient.setColorAt(1, QColor("#C62828"));
    painter.setBrush(cab_gradient);
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
    painter.drawRoundedRect(int(size * 0.1), int(size * 0.4),
                            cab_width, cab_height, 8, 8);

    QLinearGradient window_gradient(size * 0.12, size * 0.42,
                                    size * 0.12, size * 0.5);
    window_gradient.setColorAt(0, QColor("#FFFFFF"));
    window_gradient.setColorAt(1, QColor("#B0BEC5"));
    painter.setBrush(window_gradient);
    painter.drawRoundedRect(int(size * 0.12), int(size * 0.42),
                            int(size * 0.12), int(size * 0.1), 4, 4);

    QRadialGradient radial_gradient(size * 0.15, size * 0.53, size * 0.04);
    radial_gradient.setColorAt(0, QColor("#FFF176"));
    radial_gradient.setColorAt(1, QColor("#FDD835"));
    painter.setBrush(radial_gradient);
    ...
    for (double pos : wheel_positions) {
        ...
        wheel_gradient.setCenter(x_offset + wheel_radius, road_y + wheel_radius);
        wheel_gradient.setRadius(wheel_radius);
        painter.setBrush(wheel_gradient);
        painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
        painter.drawEllipse(x_offset, road_y, wheel_radius * 2, wheel_radius * 2);
        painter.setBrush(QBrush(QColor("#757575")));
        painter.drawEllipse(x_offset + int(wheel_radius * 0.6), road_y + int(wheel_radius * 0.6),
                            int(wheel_radius * 0.8), int(wheel_radius * 0.8));
    }

    QLinearGradient road_gradient(0, road_y + wheel_radius, 0, road_y + wheel_radius + size * 0.05);
    road_gradient.setColorAt(0, QColor("#757575"));
    road_gradient.setColorAt(1, QColor("#424242"));
    painter.setBrush(road_gradient);
    painter.setPen(Qt::NoPen);
    painter.drawRect(int(size * 0.05), int(road_y + wheel_radius),
...
QPixmap createNetworkManagerIcon(int size) {
    ...
    painter.drawEllipse(hub_x, hub_y, hub_radius, hub_radius);

    QVector<QPoint> nodes;
    nodes.append(QPoint(int(size * 0.15), int(size * 0.2)));
    nodes.append(QPoint(int(size * 0.8), int(size * 0.25)));
    nodes.append(QPoint(int(size * 0.75), int(size * 0.75)));
    ...
    for (const QPoint &pt : nodes) {
        QPainterPath path;
        path.moveTo(hub_x + hub_radius / 2, hub_y + hub_radius / 2);
        int control_x = (hub_x + pt.x()) / 2;
        int control_y = (hub_y + pt.y()) / 2;
        path.quadTo(control_x, control_y, pt.x() + hub_radius / 2, pt.y() + hub_radius / 2);
        painter.drawPath(path);
    }
    for (const QPoint &pt : nodes) {
        painter.setBrush(QBrush(node_color));
...
QPixmap createAutoConnectTerminalsIcon(int size) {
    ...
    painter.drawText(pixmap.rect(), Qt::AlignTop | Qt::AlignHCenter, "AN");
    painter.end();
    return pixmap;
}
...
QPixmap createConnectByInterfaceIcon(int size) {
    ...
    painter.setFont(font);
    painter.setPen(QPen(QColor("#E74C3C")));
    painter.drawText(pixmap.rect(), Qt::AlignTop | Qt::AlignHCenter, "AI");
    ...
    painter.drawRect(size / 4 - indicator_size / 2, size / 2 + indicator_size,
                     indicator_size, indicator_size);
    // Right terminal indicator
    painter.drawRect(3 * size / 4 - indicator_size / 2, size / 2 + indicator_size,
                     indicator_size, indicator_size);
    painter.end();
    return pixmap;
}
...
QPixmap createCheckNetworkIcon(int size) {
    ...
    QPen pen(connection_color, std::max(2, int(size * 0.03)));
    painter.setPen(pen);
    QList<QPair<int, int>> connections = { {0, 1}, {0, 2}, {1, 3},
                                             {2, 4}, {1, 2}, {3, 4},
                                             {3, 0}, {4, 0} };
    ...
    pen = QPen(checkmark_color, std::max(3, int(size * 0.04)));
    painter.setPen(pen);
...
QPixmap createSettingsIcon(int size) {
    ...
    for (int i = 0; i < num_teeth; ++i) {
        ...
        double x1 = center.x() + outer_radius * std::cos(angle);
        double y1 = center.y() + outer_radius * std::sin(angle);
        double x2 = center.x() + (outer_radius + tooth_height) * std::cos(angle);
        double y2 = center.y() + (outer_radius + tooth_height) * std::sin(angle);
        double x3 = center.x() + (outer_radius + tooth_height) * std::cos(angle + M_PI / num_teeth);
        double y3 = center.y() + (outer_radius + tooth_height) * std::sin(angle + M_PI / num_teeth);
        double x4 = center.x() + outer_radius * std::cos(angle + M_PI / num_teeth);
        double y4 = center.y() + outer_radius * std::sin(angle + M_PI / num_teeth);
        path.moveTo(x1, y1);
        path.lineTo(x2, y2);
        path.lineTo(x3, y3);
        path.lineTo(x4, y4);
...
QPixmap createNewProjectIcon(int size) {
    ...
    painter.setPen(QPen(QColor("#4CAF50"), std::max(1, int(size * 0.06))));
    painter.drawLine(QPointF(size * 0.4, size * 0.5), QPointF(size * 0.6, size * 0.5));
...
QPixmap createOpenProjectIcon(int size) {
    ...
    painter.setPen(QPen(QColor("#FFFFFF"), std::max(1, int(size * 0.04))));
...
QPixmap createShortestPathsIcon(int size) {
    ...
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.01))));
    QPainterPath arrow;
    arrow.moveTo(arrow_pos.x(), arrow_pos.y());
    arrow.lineTo(arrow_pos.x() - arrow_size, arrow_pos.y() - arrow_size / 2);
    arrow.lineTo(arrow_pos.x() - arrow_size, arrow_pos.y() + arrow_size / 2);
    arrow.closeSubpath();
...
QPixmap createVerifySimulationIcon(int size) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        painter.drawArc(QRect(int(size * 0.15), int(size * 0.15),
                              int(size * 0.7), int(size * 0.7)),
                        start_angle, span_angle);
    }

    painter.setPen(QPen(arrow_color, std::max(2, int(size * 0.03))));
    int arrow_size = int(size * 0.1);
    QList<int> angles = {30, 150, 270};
    for (int angle : angles) {
        double rad = angle * M_PI / 180.0;
        QPointF arrow_center(center.x() + (radius + arrow_size) * std::cos(rad),
                             center.y() + (radius + arrow_size) * std::sin(rad));
        painter.drawLine(QPointF(arrow_center.x() - arrow_size / 2, arrow_center.y()),
                         QPointF(arrow_center.x() + arrow_size / 2, arrow_center.y()));
        painter.drawLine(QPointF(arrow_center.x() + arrow_size / 2, arrow_center.y()),
                         QPointF(arrow_center.x(), arrow_center.y() - arrow_size / 3));
        painter.drawLine(QPointF(arrow_center.x() + arrow_size / 2, arrow_center.y()),
                         QPointF(arrow_center.x(), arrow_center.y() + arrow_size / 3));
    }
...
QPixmap createPanModeIcon(int size) {
    ...
    mouse_path.arcTo(QRectF(size * 0.3, size * 0.2, size * 0.4, size * 0.6), 0, 180);
    ...
    painter.drawRoundedRect(key_rect, size * 0.05, size * 0.05);
    ...
    painter.drawLine(QPointF(size * 0.5, size * 0.1),
                     QPointF(size * 0.5 - arrow_offset, size * 0.1 + arrow_offset));
    painter.drawLine(QPointF(size * 0.5, size * 0.1),
                     QPointF(size * 0.5 + arrow_offset, size * 0.1 + arrow_offset));
    // Bottom arrow
    ...
    painter.drawLine(QPointF(size * 0.5, size * 0.9),
                     QPointF(size * 0.5 - arrow_offset, size * 0.9 - arrow_offset));
    painter.drawLine(QPointF(size * 0.5, size * 0.9),
                     QPointF(size * 0.5 + arrow_offset, size * 0.9 - arrow_offset));
    // Left arrow
    ...
    painter.drawLine(QPointF(size * 0.1, size * 0.5),
                     QPointF(size * 0.1 + arrow_offset, size * 0.5 - arrow_offset));
    painter.drawLine(QPointF(size * 0.1, size * 0.5),
                     QPointF(size * 0.1 + arrow_offset, size * 0.5 + arrow_offset));
    // Right arrow
    ...
    painter.drawLine(QPointF(size * 0.9, size * 0.5),
                     QPointF(size * 0.9 - arrow_offset, size * 0.5 - arrow_offset));
    painter.drawLine(QPointF(size * 0.9, size * 0.5),
                     QPointF(size * 0.9 - arrow_offset, size * 0.5 + arrow_offset));
...
QPixmap createShowHideTerminalsIcon(int size) {
    ...
    QLinearGradient terminal_gradient(size * 0.2, size * 0.2,
                                      size * 0.2, size * 0.6);
    terminal_gradient.setColorAt(0, terminal_color);
    terminal_gradient.setColorAt(1, terminal_color.darker(120));
    painter.setBrush(terminal_gradient);
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
    painter.drawRoundedRect(int(size * 0.2), int(size * 0.2),
                            int(size * 0.4), int(size * 0.4), 8, 8);
    ...
    int eye_height = int(size * 0.15);
    QLinearGradient eye_gradient(eye_x, eye_y,
                                 eye_x, eye_y + eye_height);
    ...
    QRadialGradient pupil_gradient(eye_x + eye_width * 0.5, eye_y + eye_height * 0.5, eye_height * 0.3);
    pupil_gradient.setColorAt(0, QColor("#212121"));
    pupil_gradient.setColorAt(1, QColor("#000000"));
    painter.setBrush(pupil_gradient);
    painter.drawEllipse(int(eye_x + eye_width * 0.35), int(eye_y + eye_height * 0.2),
                        int(eye_width * 0.3), int(eye_height * 0.6));

    painter.setPen(QPen(QColor("#FFFFFF"), std::max(1, int(size * 0.02))));
    painter.drawArc(eye_x + int(eye_width * 0.1), eye_y + int(eye_height * 0.2),
                    int(eye_width * 0.2), int(eye_height * 0.3),
                    30 * 16, 120 * 16);
...
QPixmap createShowHideConnectionsIcon(int size) {
    ...
    terminal_positions.append(QPointF(size * 0.2, size * 0.2));
    terminal_positions.append(QPointF(size * 0.6, size * 0.2));
    terminal_positions.append(QPointF(size * 0.4, size * 0.5));

    painter.setPen(QPen(connection_color, std::max(2, int(size * 0.03))));
    painter.drawLine(int(terminal_positions[0].x() + size * 0.1),
    ...
    painter.drawLine(int(terminal_positions[1].x() + size * 0.1),
                     int(terminal_positions[1].y() + size * 0.1),
                     int(terminal_positions[2].x() + size * 0.1),
                     int(terminal_positions[2].y() + size * 0.1));
    ...
    for (const QPointF &pt : terminal_positions) {
        ...
        terminal_gradient.setFinalStop(pt.x(), pt.y() + terminal_size);
        painter.setBrush(terminal_gradient);
        painter.drawRoundedRect(int(pt.x()), int(pt.y()), terminal_size, terminal_size, 8, 8);
    ...
    int eye_width = int(size * 0.25);
    int eye_height = int(size * 0.15);
    QLinearGradient eye_gradient(eye_x, eye_y, eye_x, eye_y + eye_height);
    eye_gradient.setColorAt(0, eye_color);
    eye_gradient.setColorAt(1, eye_color.darker(120));
    painter.setBrush(eye_gradient);
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
...
QPixmap createShowEyeIcon(int size) {
    ...
    QLinearGradient eye_gradient(0, eye_y, 0, eye_y + eye_height);
    eye_gradient.setColorAt(0, eye_color);
    eye_gradient.setColorAt(1, eye_color.darker(120));
    QPainterPath eye_path;
    eye_path.moveTo(eye_x, eye_y + eye_height / 2);
    eye_path.quadTo(eye_x + eye_width / 2, eye_y, eye_x + eye_width, eye_y + eye_height / 2);
    eye_path.quadTo(eye_x + eye_width / 2, eye_y + eye_height, eye_x, eye_y + eye_height / 2);
    ...
    int pupil_size = int(std::min(eye_width, eye_height) * 0.5);
    int pupil_x = int(size / 2 - pupil_size / 2);
    int pupil_y = int(size / 2 - pupil_size / 2);
    QRadialGradient pupil_gradient(size / 2.0, size / 2.0, pupil_size / 2.0);
    pupil_gradient.setColorAt(0, QColor("#212121"));
    pupil_gradient.setColorAt(1, QColor("#000000"));
    painter.setBrush(pupil_gradient);
    ...
    painter.setPen(QPen(QColor("#FFFFFF"), std::max(1, int(size * 0.02))));
    painter.drawArc(int(size / 2 - pupil_size / 4), int(size / 2 - pupil_size / 4),
                    int(pupil_size / 2), int(pupil_size / 2), 30 * 16, 120 * 16);

    painter.setBrush(QColor("#FFFFFF"));
    int highlight_size = int(pupil_size * 0.15);
    painter.drawEllipse(int(size / 2 + pupil_size / 6), int(size / 2 - pupil_size / 6),
...
QPixmap createShowHidePathsTableIcon(int size) {
    ...
    QLinearGradient table_gradient(0, size * 0.2, 0, size * 0.5);
    ...
    painter.setPen(QPen(QColor("#FFFFFF"), std::max(1, int(size * 0.01))));
    int x1 = table_rect.left() + table_rect.width() / 3;
    int x2 = table_rect.left() + (table_rect.width() * 2) / 3;
    painter.drawLine(x1, table_rect.top(), x1, table_rect.bottom());
    painter.drawLine(x2, table_rect.top(), x2, table_rect.bottom());
    int y = table_rect.top() + table_rect.height() / 2;
    painter.drawLine(table_rect.left(), y, table_rect.right(), y);

    QLinearGradient path_gradient(0, 0, size * 0.1, size * 0.1);
    ...
    for (int col = 0; col < 3; ++col) {
        int x = table_rect.left() + (col * table_rect.width() / 3) + (table_rect.width() / 6);
        for (int row = 0; row < 2; ++row) {
            int y_cell = table_rect.top() + (row * table_rect.height() / 2) + (table_rect.height() / 4);
            QPainterPath path;
            path.moveTo(x - arrow_size, y_cell);
            path.lineTo(x + arrow_size, y_cell);
            path.lineTo(x + arrow_size / 2, y_cell - arrow_size / 2);
            path.moveTo(x + arrow_size, y_cell);
            path.lineTo(x + arrow_size / 2, y_cell + arrow_size / 2);
            painter.setPen(QPen(path_color, std::max(1, int(size * 0.02))));
            painter.drawPath(path);
        }
    }
...
QPixmap createThickWhiteArrowPixmap(int size, int width) {
    ...
    path.addRect(x_start, y_center - stem_height / 2,
                 width - x_start - arrow_head_width, stem_height);
    path.moveTo(width - arrow_head_width, y_center - arrow_height / 2);
    path.lineTo(width - x_start, y_center);
    path.lineTo(width - arrow_head_width, y_center + arrow_height / 2);
    path.closeSubpath();
...
QPixmap createImportTrainsIcon(int size) {
    ...
    painter.drawRoundedRect(int(size * 0.35), int(size * 0.4),
                            int(size * 0.3), int(size * 0.2), 4, 4);
...
QPixmap createDeleteTrainIcon(int size) {
    ...
    painter.setPen(QPen(Qt::black, std::max(1, int(size * 0.02))));
    painter.drawRoundedRect(int(size * 0.2), int(size * 0.3),
                            int(size * 0.6), int(size * 0.4), 8, 8);

    painter.setPen(QPen(QColor("#F44336"), std::max(3, int(size * 0.06))));
    painter.drawLine(int(size * 0.65), int(size * 0.25),
...
QPixmap createTrainManagerIcon(int size) {
    ...
    QLinearGradient train_gradient(0, size * 0.3, 0, size * 0.7);
    train_gradient.setColorAt(0, train_color);
    train_gradient.setColorAt(1, train_color.darker(120));
    painter.setBrush(train_gradient);
    painter.drawRoundedRect(int(size * 0.25), int(size * 0.35),
                            int(size * 0.5), int(size * 0.25), 8, 8);
    ...
    painter.setPen(QPen(rail_color, std::max(2, int(size * 0.03))));
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        double x = size * 0.5 + size * 0.45 * std::cos(angle);
        double y = size * 0.5 + size * 0.45 * std::sin(angle);
        painter.drawEllipse(int(x - size * 0.08),
                            int(y - size * 0.08),
...
QPixmap createShipManagerIcon(int size) {
    ...
    QLinearGradient water_gradient(0, size * 0.6, 0, size * 0.8);
    ...
    QLinearGradient ship_gradient(0, size * 0.3, 0, size * 0.6);
...
// Set Global Position Icon
//------------------------------------------------------------------------------
QPixmap createTransportationModePixmap(const QString& mode, int size, int width) {
    // Create a pixmap for the transportation mode with text and arrow
    ...
    if (mode.contains("Truck", Qt::CaseInsensitive)) {
        modeColor = QColor(255, 0, 255);  // Magenta for truck
    } else if (mode.contains("Rail", Qt::CaseInsensitive) ||
               mode.contains("Train", Qt::CaseInsensitive)) {
        modeColor = QColor(80, 80, 80);   // Dark gray for rail
    ...
    font.setBold(true);
    painter.setFont(font);
    painter.drawText(QRect(0, 0, pixmap.width(), 15), Qt::AlignCenter, mode);
    ...
    // Arrow head
    QPolygon arrowHead;
    arrowHead << QPoint(width - 16, 20) << QPoint(width - 10, 25) << QPoint(width - 16, 30);
