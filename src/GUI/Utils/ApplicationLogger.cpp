...
LogEvent::LogEvent(const LogEntry& entry)
    : QEvent(LogEventType), entry(entry)
{
}

// ProgressEvent implementation
ProgressEvent::ProgressEvent(float value, int clientIndex)
    : QEvent(ProgressEventType), value(value), clientIndex(clientIndex)
{
}
...
ApplicationLogger::ApplicationLogger() :
    ...
    // Move to the main thread if created in a different thread
...
void ApplicationLogger::log(const QString& message, int clientType) {
    logMessageInternal(message, clientType, false);
}

void ApplicationLogger::logError(const QString& message, int clientType) {
    logMessageInternal(message, clientType, true);
}

void ApplicationLogger::updateProgress(float progressValue, int clientType) {
    ...
    s_progressQueue.enqueue(qMakePair(progressValue, clientType));
    ...
    if (QThread::currentThread() != getInstance()->thread()) {
        ...
        emit getInstance()->progressUpdated(progress, clientType);
    }
}
...
void ApplicationLogger::logMessageInternal(const QString& message,
    ...
    // Post event to main thread if not already there
    if (QThread::currentThread() != getInstance()->thread()) {
        QApplication::postEvent(getInstance(), new LogEvent(entry));
    } else {
        // Process immediately if already in the main thread
...
void ApplicationLogger::appendLogEntry(const LogEntry& entry) {
    // Format timestamp
    QDateTime dateTime = QDateTime::fromMSecsSinceEpoch(entry.timestamp);
    QString timeStr = dateTime.toString("yyyy-MM-dd hh:mm:ss.zzz");
    
    // Create formatted log message
    QString formattedMessage = QString("[%1] %2").arg(timeStr, entry.message);
    
    // Add to client logs
    m_clientLogs[entry.clientIndex].append(formattedMessage);
    
    // Emit signal with the new message
    emit newLogMessage(entry.message, entry.clientIndex, entry.isError);
}

bool ApplicationLogger::saveLogsToFile(const QString& filePath) {
    QFile file(filePath);
    ...
    for (int clientIndex = 0; clientIndex < 5; ++clientIndex) {
        if (m_clientLogs[clientIndex].isEmpty()) {
            continue;
        }
        
        out << "=== " << clientNames[clientIndex] << " Logs ===" << Qt::endl;
        
        foreach (const QString& logEntry, m_clientLogs[clientIndex]) {
