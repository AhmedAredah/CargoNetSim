...
public:
     ...
     * @param isError Flag indicating if this is an error
     * message
    ...
    int     clientIndex; ///< The client that generated the
                         ///< message
    bool isError; ///< Flag indicating if this is an error
                  ///< message
...
public:
     ...
     * infinite)
     * @return True if initialization completed, false if
     * timed out
     */
...
signals:
     ...
     */
    void progressUpdated(int progressValue,
                         int clientIndex);
...
private:
    ...
    static QMutex
        s_progressMutex; ///< Mutex for progress operations
    static QWaitCondition
        s_initCondition; ///< Condition for initialization
                         ///< waiting
    ...
    static QQueue<QPair<float, int>>
        s_progressQueue; ///< Queue of pending progress
                         ///< updates
    ...
    QMap<int, int>
        m_clientProgress; ///< Current progress by client

    bool m_isRunning; ///< Flag indicating if logger is
                      ///< running
