...
public:
    ...
    *         found.
    */
    NeTrainSimNetworkBase* getTrainNetwork(
    ...
    *         found.
    */
    IntegrationNetworkBase* getTruckNetwork(
        const QString& name) const;
...
public:
    ...
    */
    static RegionDataController& getInstance(
        QObject* parent = nullptr);
    ...
    */
    ...
    RegionDataController(
        const RegionDataController&) = delete;
    RegionDataController& operator=(
        const RegionDataController&) = delete;
    ...
    */
    bool setRegionVariable(const QString& regionName,
        ...
        if (region) {
            ...
            emit regionVariableChanged(
                regionName, key, value);
            return true;
        }
        return false;
    }
    ...
    */
    ...
    T getGlobalVariableAs(
        const QString& key,
        const T& defaultValue = T()) const {
        QVariant var = m_globalVariables.value(
            key, QVariant());
        if (var.isValid() && var.canConvert<T>()) {
            return var.value<T>();
        }
    ...
    /**
    * @brief Get a specific variable from all m_regions with
    *        its region name as key.
    * @tparam T The type to convert the value to.
    * @param key The name/key of the variable to retrieve.
    ...
    */
    template <typename T>
    QMap<QString, T> getAllRegionVariableAs(
...
Q_DECLARE_METATYPE(CargoNetSim::Backend::RegionData*)
Q_DECLARE_METATYPE(CargoNetSim::Backend::RegionDataController)
Q_DECLARE_METATYPE(CargoNetSim::Backend::RegionDataController*)
