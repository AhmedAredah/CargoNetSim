...
TrainState::TrainState(const QJsonObject& trainData)
    : m_cumEnergyStat(trainData["cumEnergyStat"].toDouble()),
      ...
      m_cumulativeMaxDelayTime(
          trainData["cumulativeMaxDelayTimeStat"].toDouble()),
      ...
      m_trainUserId(trainData["trainUserID"].toString("Unknown")),
      m_travelledDistance(
          trainData["travelledDistance"].toDouble()),
      m_containersCount(trainData["containersCount"].toInt())
{
    // Extract fuel consumption data from JSON
    QJsonObject fuelObj = trainData["totalFuelConsumed"].toObject();

    // Iterate over fuel types and their values
    for (auto it = fuelObj.begin(); it != fuelObj.end(); ++it) {
        // Store each fuel type and its consumption
        m_totalFuelConsumed[it.key()] = it.value().toDouble();
    }
    ...
    m_metrics["cumulativeDelayTime"] = m_cumulativeDelayTime;
    m_metrics["cumulativeMaxDelayTime"] = m_cumulativeMaxDelayTime;
    m_metrics["cumulativeStoppedStat"] = m_cumulativeStoppedStat;
    m_metrics["tripTime"] = m_tripTime;
    m_metrics["currentAcceleration"] = m_currentAcceleration;
    m_metrics["currentResistanceForces"] = m_currentResistanceForces;
    m_metrics["currentSpeed"] = m_currentSpeed;
    m_metrics["currentTractiveForce"] = m_currentTractiveForce;
    m_metrics["currentUsedTractivePower"] = m_currentUsedTractivePower;
    ...
    m_metrics["totalEnergyConsumed"] = m_totalEnergyConsumed;
    m_metrics["totalEnergyRegenerated"] = m_totalEnergyRegenerated;
    m_metrics["totalCarbonDioxideEmitted"] = m_totalCarbonDioxideEmitted;
...
QVariant TrainState::getMetric(const QString& metricName) const
{
...
QJsonObject TrainState::toJson() const
{
    ...
    obj["totalEnergyRegenerated"] = m_totalEnergyRegenerated;
    obj["totalCarbonDioxideEmitted"] = m_totalCarbonDioxideEmitted;
