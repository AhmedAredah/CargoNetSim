 ...
 * @date March 20, 2025
 *
 * Implements the SimulationResults class, managing simulation
 * results with extensive comments for auditing and review.
 */
...
SimulationResults::SimulationResults(
    ...
    : m_summaryData(summaryData),             // Init summary data
      m_trajectoryFileData(trajectoryFileData),// Init file data
      m_trajectoryFileName(trajectoryFileName),// Init file name
      m_summaryFileName(summaryFileName)      // Init summary name
{
    // Constructor initializes all member variables directly
    // using the provided parameters, no additional logic needed
}

SimulationResults SimulationResults::fromJson(const QJsonObject& jsonObj)
{
    ...
    if (jsonObj["trajectoryFileDataIncluded"].toBool()) {
        ...
        QString base64Data = jsonObj["trajectoryFileData"].toString();

        // Decode base64 string to raw byte data
        trajectoryFileData = QByteArray::fromBase64(base64Data.toUtf8());
    }

    // Extract trajectory file name from JSON, default to empty
    QString trajectoryFileName = jsonObj["trajectoryFileName"].toString();

    // Extract summary file name from JSON, default to empty
    QString summaryFileName = jsonObj["summaryFileName"].toString();
...
{
    // Split the file name by '/' to separate path components
