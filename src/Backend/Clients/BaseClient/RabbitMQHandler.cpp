...
bool RabbitMQHandler::establishConnection()
{
    ...
    while (retryCount < MAX_RETRIES) {
        try {
            ...
            if (!m_sendConnection) {
                qWarning() << "Failed to create send connection";
                retryCount++;
            ...
            if (!m_receiveConnection) {
                qWarning() << "Failed to create receive connection";
                amqp_destroy_connection(m_sendConnection);
                m_sendConnection = nullptr;
                retryCount++;
                std::this_thread::sleep_for(
...
{
    qDebug() << "Attempting to reconnect sending connection";
    ...
    m_sendConnection = amqp_new_connection();
    if (!m_sendConnection) {
        qWarning() << "Failed to create new send connection";
        return;
    }
    ...
    if (loginReply.reply_type != AMQP_RESPONSE_NORMAL) {
        qWarning() << "Failed to login to new send connection";
        amqp_destroy_connection(m_sendConnection);
        m_sendConnection = nullptr;
        return;
    }
    ...
    qDebug() << "Successfully reconnected sending connection";
}
...
{
    qDebug() << "Attempting to reconnect receiving connection";
    ...
    m_receiveConnection = amqp_new_connection();
    if (!m_receiveConnection) {
        qWarning() << "Failed to create new receive connection";
        return;
    }
    ...
    if (loginReply.reply_type != AMQP_RESPONSE_NORMAL) {
        qWarning() << "Failed to login to new receive connection";
        amqp_destroy_connection(m_receiveConnection);
        m_receiveConnection = nullptr;
        return;
    }
    ...
    qDebug() << "Successfully reconnected receiving connection";
}
