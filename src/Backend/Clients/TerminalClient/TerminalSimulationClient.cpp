...
TerminalSimulationClient::~TerminalSimulationClient()
{
    ...
    for (const QList<PathSegment*>& segments : m_shortestPaths.values()) {
    ...
    for (const QList<ContainerCore::Container*>& containers :
         m_containers.values()) {
        for (ContainerCore::Container* container : containers) {
            delete container; // Only if ownership transferred
        }
    }
    m_containers.clear();
    ...
    const QString& terminalId)
{
    ...
    return m_terminalAliases.value(terminalId, QStringList());
}
...
// Add route
bool TerminalSimulationClient::addRoute(const PathSegment* route)
{
...
bool TerminalSimulationClient::connectTerminalsByInterfaceModes()
{
...
bool TerminalSimulationClient::connectTerminalsInRegionByMode(
...
bool TerminalSimulationClient::connectRegionsByMode(int mode)
{
...
// Find shortest path
QList<PathSegment*> TerminalSimulationClient::findShortestPath(
    const QString& start,
    const QString& end,
    int mode)
{
    ...
    return m_shortestPaths.value(key, QList<PathSegment*>());
}
    ...
    bool skipDelays)
{
    ...
    executeSerializedCommand([&]() {
        ...
        params["n"] = n;
        params["mode"] = mode;
        params["skip_same_mode_terminal_delays_and_costs"] = skipDelays;
    ...
    const QString& condition)
{
    ...
    QMutexLocker locker(&m_dataMutex);
    return m_containers.value(terminalId, QList<ContainerCore::Container*>());
}
...
QList<ContainerCore::Container*>
TerminalSimulationClient::dequeueContainersByNextDestination(
    const QString& terminalId,
    const QString& destination)
{
...
// Ping server
QJsonObject TerminalSimulationClient::ping(const QString& echo)
{
    ...
    const QJsonObject& message)
{
    ...
    if (normEvent == "terminaladded") {
        ...
        if (m_logger) {
            m_logger->logError(
                QString("Unknown event received:%1").arg(event),
                static_cast<int>(m_clientType));
        } else {
            // Log unknown events for debugging
            qWarning() << "Unknown event received:" << event;
        }
    }
    ...
    const QJsonObject& message)
{
    ...
    QMutexLocker locker(&m_dataMutex);
    Terminal* existing = m_terminalStatus.value(name, nullptr);
    if (existing) {
        delete existing;
    }
    ...
    const QJsonObject& message)
{
    ...
    if (params.contains("n")) {
        ...
        for (const QJsonValue& pathVal : result) {
            ...
            QJsonArray segArray = pathObj["segments"].toArray();
            for (const QJsonValue& segVal : segArray) {
                QJsonObject segObj = segVal.toObject();
                PathSegment* segment = new PathSegment(
                    QString("seg_%1").arg(rand()), // Temporary ID
                    segObj["from"].toString(),
                    segObj["to"].toString(),
                    segObj["mode"].toInt(),
                    segObj["attributes"].toObject(),
                    this);
                segments.append(segment);
            }
            QList<QJsonObject> terminals;
            QJsonArray termArray = pathObj["terminalsInPath"].toArray();
        ...
        QList<PathSegment*> oldSegments = m_shortestPaths.value(key);
    ...
    const QJsonObject& message)
{
    ...
    for (const QList<ContainerCore::Container*>& containers : m_containers.values()) {
        for (ContainerCore::Container* container : containers) {
            delete container;
        }
    }
    ...
    const QJsonObject& message)
{
    ...
    QList<ContainerCore::Container*> oldContainers = m_containers.value(terminalId);
    if (isDequeue) {
    ...
    for (const QJsonValue& val : containers) {
        ContainerCore::Container* container = new ContainerCore::Container(val.toObject());
