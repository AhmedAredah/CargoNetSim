...
Locomotive::Locomotive(const QJsonObject& json, QObject* parent)
    : QObject(parent)
{
    ...
    , m_optimize(optimize)
{
    ...
    for (auto locomotive : m_locomotives) {
        ...
        connect(locomotive, &Locomotive::locomotiveChanged, this, &Train::trainChanged);
    }
    
    for (auto car : m_cars) {
        car->setParent(this);
        connect(car, &Car::carChanged, this, &Train::trainChanged);
    }
}
    ...
    : QObject(parent)
{
    ...
    // Parse train path
    QJsonArray pathArray = json["TrainPathOnNodeIDs"].toArray();
    ...
    for (int i = 0; i < locoArray.size(); ++i) {
        Locomotive* locomotive = new Locomotive(locoArray[i].toObject(), this);
...
void Train::setTrainPathOnNodeIds(const QVector<int>& trainPathOnNodeIds) {
...
void Train::setLocomotives(const QVector<Locomotive*>& locomotives) {
    // Disconnect old signals
    for (auto locomotive : m_locomotives) {
        disconnect(locomotive, &Locomotive::locomotiveChanged, this, &Train::trainChanged);
    }
...
void Train::setCars(const QVector<Car*>& cars) {
    // Disconnect old signals
    for (auto car : m_cars) {
        disconnect(car, &Car::carChanged, this, &Train::trainChanged);
    }
...
QVector<Train*> TrainsReader::readTrainsFile(const QString& filePath, QObject* parent) {
    QVector<Train*> trains;
    
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qCritical() << "Error: Trains file" << filePath << "does not exist.";
        return trains;
    }
    ...
    if (lines.isEmpty()) {
        qCritical() << "Error: Trains file" << filePath << "is empty!";
        return trains;
    }
    ...
    for (int i = 2; i < lines.size(); ++i) {
        ...
        QStringList lv = line.split(QRegularExpression("\\t+"));
        
        if (lv.size() != 6) {
            qCritical() << "Error: Trains file has a wrong structure.";
            qDeleteAll(trains);
            return QVector<Train*>();
        }
        
        // Parse train path, locomotives, and cars
        QVector<int> trainPathOnNodeIds = splitStringToIntList(lv[1]);
        QVector<Locomotive*> locomotives = parseLocomotives(lv[4], parent);
...
QVector<Locomotive*> TrainsReader::parseLocomotives(const QString& locomotivesStr, QObject* parent) {
...
QVector<Car*> TrainsReader::parseCars(const QString& carsStr, QObject* parent) {
    QVector<Car*> cars;
...
QVector<int> TrainsReader::splitStringToIntList(const QString& string) {
    QVector<int> result;
