    ...
    m_segments(segments)
{
    ...
    if (id <= 0) {
        ...
        throw std::invalid_argument("Path ID must be positive");
    }
    ...
    if (segments.isEmpty()) {
        ...
        throw std::invalid_argument("Path must have segments");
    }
    ...
    if (terminals.size() != expectedTerminals) {
        ...
        << "does not match expected" << expectedTerminals;
    }

    // No ownership transfer; segments managed by this instance
}
...
Path::~Path()
{
    ...
    for (PathSegment* segment : m_segments) {
        ...
        delete segment;
        // Nullify pointer for safety (not required but explicit)
        segment = nullptr;
    }
