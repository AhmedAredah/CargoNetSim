...
* @brief Base class for directed graph implementations in the
*        CargoNetSim backend.
* @author Ahmed Aredah
*/
...
* @brief Base class that provides a Qt-based implementation of a
* directed graph.
*
* DirectedGraphBase serves as an abstract base class for all directed
* graph implementations within the CargoNetSim backend. It provides a
* signal-based interface for graph modification events, allowing for reactive
* UI updates and event-driven programming patterns.
*
* Derived classes should implement the actual graph data structure
* and operations, emitting the appropriate signals when the graph
* structure changes.
*
* @note This class uses QVariant for node and edge identifiers to
* provide flexibility in the types of identifiers that can be used.
* Implementing classes should document the expected format of these
* identifiers.
*/
...
signals:
    /**
    * @brief Emitted when any change occurs in the graph structure.
    *
    * This is a general signal that indicates some change has
    * occurred in the graph. For more specific information about
    * what changed, listen to the other signals.
    */
    void graphChanged();
    ...
    */
    ...
    /**
    * @brief Emitted when an edge is removed from the graph.
    * @param fromNodeId The identifier of the source node.
    * @param toNodeId The identifier of the target node.
    */
    ...
    /**
    * @brief Emitted when an edge's properties are modified.
    * @param fromNodeId The identifier of the source node.
    * @param toNodeId The identifier of the target node.
    */
