 ...
 * @brief Template implementation of a directed graph with
 ...
 * @brief Custom priority queue entry for the shortest path
 * algorithm.
...
/**
 * @class DirectedGraph
 * @brief A template implementation of a directed graph with
 * node and edge attributes.
 *
 * This class provides a comprehensive implementation of a
 ...
 *
 * @tparam T The type of node identifier, which must be
...
public:
     ...
     * @param attributes Key-value pairs of node attributes
     * (default: empty).
     */
    void addNode(const T                       &nodeId,
                 const QMap<QString, QVariant> &attributes =
                     QMap<QString, QVariant>());

    /**
     * @brief Adds a directed edge with weight and optional
     * attributes.
     * @param fromNodeId The source node identifier.
     * @param toNodeId The target node identifier.
     * @param weight The edge weight (used for path
     * calculations).
     * @param attributes Key-value pairs of edge attributes
     * (default: empty).
     */
    void addEdge(const T &fromNodeId, const T &toNodeId,
                 float                          weight,
                 const QMap<QString, QVariant> &attributes =
                     QMap<QString, QVariant>());
     ...
     */
    bool hasEdge(const T &fromNodeId,
     ...
     */
    QMap<QString, QVariant>
    getNodeAttributes(const T &nodeId) const;
     ...
     */
    QMap<QString, QVariant>
    getEdgeAttributes(const T &fromNodeId,
                      const T &toNodeId) const;
     ...
     * @param fromNodeId The source node identifier.
     * @param toNodeId The target node identifier.
     * @return The edge weight, or infinity if the edge
     * doesn't exist.
     */
    float getEdgeWeight(const T &fromNodeId,
                        const T &toNodeId) const;
     ...
     */
    void setEdgeWeight(const T &fromNodeId,
                       const T &toNodeId, float weight);
     ...
     * @return A vector of pairs containing target node
     * identifiers and edge weights.
     */
    QVector<QPair<T, float>>
    getOutgoingEdges(const T &nodeId) const;

    /**
     * @brief Gets all incoming edges to a node.
     * @param nodeId The target node identifier.
     * @return A vector of pairs containing source node
     * identifiers and edge weights.
     */
    QVector<QPair<T, float>>
    getIncomingEdges(const T &nodeId) const;

    /**
     * @brief Gets the out-degree of a node (number of
     * outgoing edges).
     * @param nodeId The node identifier.
    ...
    /**
     * @brief Gets the in-degree of a node (number of
     * incoming edges).
     * @param nodeId The node identifier.
     ...
     * (default: "distance").
     * @return A vector of node identifiers representing the
     * shortest path, or empty if no path exists.
     */
    QVector<T> findShortestPath(
...
private:
    /**
     * @brief Calculates the cost of an edge based on the
     ...
     */
    float
    calculateEdgeCost(const T       &fromNodeId,
                      const T       &toNodeId,
    ...
    QMap<T, QMap<T, QMap<QString, QVariant>>>
        m_edgeAttributes;
...
DirectedGraph<T>::DirectedGraph(QObject *parent)
    ...
    static_assert(
        ...
        "Type T must be storable in QVariant for "
        "signal/slot usage");
}
...
void DirectedGraph<T>::addEdge(
    ...
    bool edgeExists =
        m_edgeWeights.contains(fromNodeId)
...
void DirectedGraph<T>::removeEdge(const T &fromNodeId,
    ...
    emit edgeRemoved(QVariant::fromValue(fromNodeId),
                     QVariant::fromValue(toNodeId));
    emit graphChanged();
}
...
QVector<T> DirectedGraph<T>::findShortestPath(
    ...
    for (const T &nodeId : getNodes()) {
        ...
        // We need to initialize with a valid value of type
        // T
        predecessors[nodeId] =
            nodeId; // Temporarily point to self
    }
    ...
    while (!pq.isEmpty()) {
        ...
        auto it = std::min_element(
            pq.begin(), pq.end(),
            [](const PriorityQueueEntry<T> &a,
               const PriorityQueueEntry<T> &b) {
        ...
        for (const QPair<T, float> &edge :
            ...
            // Calculate cost based on optimization
            // criterion
            float edgeCost = calculateEdgeCost(
                currentNode, neighborId, optimizeFor);
...
QJsonObject DirectedGraph<T>::toJson() const {
    ...
    for (auto fromIt = m_edgeWeights.constBegin();
        ...
        for (auto toIt = fromIt.value().constBegin();
            ...
            QJsonObject edgeObj;
            edgeObj["from"] = QJsonValue::fromVariant(
                QVariant::fromValue(fromNodeId));
            edgeObj["to"] = QJsonValue::fromVariant(
                QVariant::fromValue(toNodeId));
...
Q_DECLARE_METATYPE(
    CargoNetSim::Backend::DirectedGraph<int> *)
Q_DECLARE_METATYPE(
    CargoNetSim::Backend::DirectedGraph<QString>)
Q_DECLARE_METATYPE(
    CargoNetSim::Backend::DirectedGraph<QString> *)
